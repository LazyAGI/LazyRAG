...
def api_health():
    """Health check; shows if bootstrap has run (admin and user roles exist)."""
...
# ---------- Permission groups (read-only, from YAML) ----------
@app.get("/api/auth/permission-groups")
def api_list_permission_groups():
    """List built-in permission groups (category.action). Not modifiable at runtime."""
    with SessionLocal() as db:
        groups = db.scalars(select(PermissionGroup).order_by(PermissionGroup.name)).all()
    return [{"id": g.id, "name": g.name} for g in groups]


# ---------- Roles (admin can add/delete non-built-in only) ----------
...
def api_set_user_role(user_id: int, body: UserRoleBody, _: User = Depends(_require_admin)):
    with SessionLocal() as db:
        ...
        if user.role.name == BUILTIN_ADMIN_ROLE and user.role.built_in:
            raise HTTPException(status_code=400, detail="Admin account role cannot be changed")
        role = db.scalar(select(Role).where(Role.id == body.role_id))
        if not role:
            raise HTTPException(status_code=404, detail="Role not found")
