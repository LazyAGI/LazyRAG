...
from evo.domain.clustering import ClusterSummary, ClusteringResult, FlowAnalysisResult, PerStepClusteringResult, PerStepSummary, StepTransition
...
def _build_cluster_summaries(ids: list[str], keys: list[str], mat: np.ndarray, labels: np.ndarray, score_lookup: Callable[[str], float | None]) -> list[ClusterSummary]:
    ...
    for lab in sorted(set(labels)):
        ...
        out.append(ClusterSummary(cluster_id=f'cluster_{lab}' if lab >= 0 else 'noise', size=int(mask.sum()), score_stats={'mean': round(float(np.mean(scores)), 4) if scores else None, 'min': round(float(min(scores)), 4) if scores else None, 'max': round(float(max(scores)), 4) if scores else None}, top_feature_deltas={keys[i]: round(float(deltas[i]), 3) for i in top_idx}, step_grouped_deltas=step_deltas, exemplar_case_ids=exemplars))
    return out
def cluster_badcases(score_field: str='answer_correctness', order: str='asc', limit: int=500, method: str='', min_cluster_size: int=0) -> ToolResult[ClusteringResult]:
    ...
    if not frows:
        for (did, judge) in session.iter_judge():
            ...
            frows.append({'judge:answer_correctness': judge.answer_correctness, 'judge:context_recall': judge.context_recall, 'judge:doc_recall': judge.doc_recall, 'judge:faithfulness': judge.faithfulness, 'judge:key_hit_rate': len(judge.hit_key) / key_count, 'judge:retrieved_contexts': float(len(judge.retrieved_text)), 'judge:retrieved_docs': float(len(judge.retrieved_file))})
    ...
    if not keep:
        return ToolResult.failure('cluster_badcases', ErrorCode.INTERNAL_ERROR, 'No features matched the ranked target set.')
    ...
    result = ClusteringResult(method=effective_method, n_cases=len(ids), n_clusters=len([c for c in set(labels) if c >= 0]), noise_count=int((labels == -1).sum()) if -1 in labels else 0, cluster_summaries=summaries)
    return ToolResult.success('cluster_badcases', result)
def cluster_per_step(score_field: str='answer_correctness', limit: int=500, method: str='', min_cluster_size: int=0) -> ToolResult[PerStepClusteringResult]:
    if limit < 1:
    ...
    for step_key in pipeline:
        ...
        per_step[step_key] = PerStepSummary(n_cases=len(ids), n_clusters=len([c for c in set(labels) if c >= 0]), cluster_summaries=summaries, labels={cid: int(lab) for (cid, lab) in zip(ids, labels)})
...
def analyze_step_flow() -> ToolResult[FlowAnalysisResult]:
    ...
    for (s_a, s_b) in edges:
        ...
        transitions.append(StepTransition(from_step=s_a, to_step=s_b, entropy_from=round(h_a, 3), entropy_to=round(h_b, 3), entropy_change=round(delta_h, 3), nmi=round(nmi_val, 3), type=_classify_transition(delta_h, nmi_val), transition_matrix=tmat.tolist(), from_clusters=[f'cluster_{c}' for c in unique_a], to_clusters=[f'cluster_{c}' for c in unique_b]))
    ...
    for cid in all_cids[:50]:
        ...
        case_flow[cid] = flow
    return ToolResult.success('analyze_step_flow', FlowAnalysisResult(transition_analysis=transitions, critical_steps=critical, case_label_flow=case_flow))
