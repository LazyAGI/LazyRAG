...
def load_corpus(session: AnalysisSession, judge_path: Path | None=None, trace_path: Path | None=None) -> LoadSummary:
    ...
    session.set_parsed_corpus(judges=judges, traces=traces, trace_meta=trace_meta, warnings=warnings, eval_report_meta=eval_meta)
    return LoadSummary(total_cases=len(judges), field_histogram={}, sample_keys=list(judges)[:5], warnings=warnings[:], missing_traces=trace_missing or None)
