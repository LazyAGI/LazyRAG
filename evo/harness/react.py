...
def _runtime_context(session: AnalysisSession) -> str:
    global_clusters = [cs.cluster_id for cs in (session.clustering_global.cluster_summaries if session.clustering_global else [])]
    per_step_clusters = {step: [cs.cluster_id for cs in result.cluster_summaries] for (step, result) in (session.clustering_per_step.per_step.items() if session.clustering_per_step else [])}
    score_fields = _numeric_judge_fields(session)
    payload = {'global_cluster_ids': global_clusters, 'per_step_cluster_ids': per_step_clusters, 'step_keys': list(session.trace_meta.pipeline), 'score_fields': score_fields, 'sample_dataset_ids': session.sample_dataset_ids(8)}
    return f'## 当前运行的工具参数边界（强约束）\n{json.dumps(payload, ensure_ascii=False, indent=2)}\n- cluster_id 必须逐字来自 global_cluster_ids 或对应 step 的 per_step_cluster_ids；禁止使用占位符、空值或自行编造。\n- step_key 必须逐字来自 step_keys；score_field 必须逐字来自 score_fields。\n- limit/k/offset 必须是整数，threshold 必须是数字。\n- dataset_id 必须来自 sample_dataset_ids，或来自你实际调用过的 list_bad_cases / list_cases_ranked / list_cluster_exemplars 返回值。'
...
_FORMAT_VIOLATION_RES: tuple[re.Pattern, ...] = (re.compile('\\[TOOL_CALL\\]'), re.compile('<invoke\\b'), re.compile('<\\w+:tool_call\\b'), re.compile('"\\s*(?:tool|name|function|tool_name)\\s*"\\s*:\\s*"'), re.compile('```tool_code'))
def _parse_action(text: str) -> tuple[str, dict[str, Any]] | None:
    ...
    if invoke is not None:
        ...
        for (name, value) in re.findall('<parameter\\s+name=["\\\'](\\w+)["\\\']\\s*>(.*?)</parameter>', body, flags=re.DOTALL):
...
class ReActRunner:
    def __init__(self, session: AnalysisSession, tool_names: list[str], invoker: 'LLMInvoker', *, agent: str='react', logger: logging.Logger | None=None, cfg: ReActConfig | None=None) -> None:
    ...
    def run(self, task: str) -> str:
        ...
        self.session.telemetry.emit('researcher.started', actor=self.agent, task=task, tools=[s.name for s in self.specs])
        for round_idx in range(self.cfg.max_rounds):
            ...
            self.session.telemetry.emit('researcher.turn.completed', actor=self.agent, round=round_idx + 1, prompt=prompt, response=response, elapsed_s=round(time.monotonic() - t0, 4))
            self.stats.rounds = round_idx + 1
            self.log.info('ReAct round %d (%.2fs, prompt=%d chars, turns=%d)', round_idx + 1, time.monotonic() - t0, len(prompt), len(turns))
            ...
            if action is None:
                ...
                if violations and self.stats.finish_warnings < self.cfg.max_finish_warnings:
                    ...
                    self.log.info('Finish blocked (%s); warning %d/%d', ', '.join(violations), self.stats.finish_warnings, self.cfg.max_finish_warnings)
                    continue
                self.session.telemetry.emit('researcher.reasoning_summary', actor=self.agent, rounds=self.stats.rounds, tool_calls=dict(self.stats.tool_calls), final_answer=response)
                return response
            (tool_name, args) = action
            spec = spec_map.get(tool_name)
            t_tool = time.monotonic()
            if spec is None:
                self.session.telemetry.emit('researcher.tool_call.failed', actor=self.agent, round=round_idx + 1, tool=tool_name, args=args, error='unknown tool')
                hints.clear()
                hints.append(f'工具 `{tool_name}` 不在可用工具列表中。只能从这些工具中选择：{sorted(spec_map)}')
                continue
            else:
                args = _normalize_args(spec, args)
                self.session.telemetry.emit('researcher.tool_call.started', actor=self.agent, round=round_idx + 1, tool=tool_name, args=args)
                result = spec.fn(**args) if args else spec.fn()
            ok = result.ok
            summary = spec.summarize_result(result) if spec else f'FAIL unknown tool {tool_name}'
            self.session.telemetry.emit('researcher.tool_call.completed', actor=self.agent, round=round_idx + 1, tool=tool_name, args=args, ok=ok, handle=result.handle, output=_tool_result_payload(result), summary=summary, elapsed_s=round(time.monotonic() - t_tool, 4))
            if not ok:
                hints.clear()
                hints.append(f'工具 `{tool_name}` 参数无效或未命中：{summary}。请检查上一条 Observation 中的真实 handle/dataset_id/cluster_id，或换用 list_bad_cases / list_cases_ranked / list_cluster_exemplars 获取真实 ID。')
                turns.append(_Turn(response=response, tool=tool_name, args=args, obs=json.dumps({'ok': False, 'summary': summary}, ensure_ascii=False), ok=False, summary=summary))
                continue
            ...
            self.session.telemetry.emit('researcher.observation', actor=self.agent, round=round_idx + 1, tool=tool_name, observation=obs_payload)
            truncated = False
            ...
            self.session.telemetry.emit('tool_call', agent=self.agent, tool=tool_name, args_keys=sorted(args.keys()), ok=ok, handle=result.handle, elapsed_s=round(time.monotonic() - t_tool, 4), out_chars=len(obs), truncated=truncated)
            turns.append(_Turn(response=response, tool=tool_name, args=args, obs=obs, ok=ok, summary=summary))
            if curator is not None:
                working_memory = curator.update(working_memory, tool=tool_name, args_brief=_args_brief(args), summary=summary, handle=result.handle, ok=ok)
            ...
            if same_streak >= self.cfg.same_streak_warn:
                ...
                hints.append(f'提示：你已经连续 {same_streak} 次调用同一个工具 `{tool_name}`，再次调用前请改变参数或换一个工具。')
        ...
        self.session.telemetry.emit('researcher.reasoning_summary', actor=self.agent, rounds=self.stats.rounds, tool_calls=dict(self.stats.tool_calls), final_answer=last_response, exhausted=True)
    ...
    def _format_finish_hint(self, violations: list[str]) -> str:
        ...
        if any(('non-standard' in v for v in violations)):
            parts.append('本系统【唯一】支持的工具调用格式是：\nThought: <thought>\nAction: <tool_name>\nAction Input: {<json>}\n请勿使用 [TOOL_CALL]{...} / <invoke> / <tool_call> 等其它格式；若已收集到足够证据则直接输出最终 JSON 结果（不要再写 Action）。')
        else:
            parts.append('请继续调用相关工具完善证据；')
        parts.append(f'再连续 {remaining} 次违规将允许放行。')
        return ' '.join(parts)
    def _build_prompt(self, header: str, turns: list[_Turn], hints: list[str], working_memory: str, round_idx: int) -> str:
        parts = [header]
        if working_memory:
        ...
        if not self.cfg.use_memory_curator and n > keep:
            parts.append('## 历史摘要（参数与摘要）\n' + '\n'.join((f'  - round {i + 1}: {t.tool}({_args_brief(t.args)}) -> {t.summary}' for (i, t) in enumerate(turns[:n - keep]))))
...
class LLMInvoker:
    ...
    def _build_llm(self) -> Any:
        ...
        if provider is None:
            raise RuntimeError('No llm_provider on session; pass llm_provider=... to create_session() or RAGAnalysisPipeline.')
        llm = provider()
        if llm is None:
    ...
    def invoke(self, user_text: str, *, system_prompt: str | None=None) -> str:
        ...
        full_prompt = f'{sp}\n\n---\n\n{user_text}'
        self.session.telemetry.emit('llm.prompt', actor='llm', system_prompt=sp, user_text=user_text, full_prompt=full_prompt)
        t0 = time.monotonic()
        try:
        ...
        self.session.telemetry.emit('llm.answer', actor='llm', answer=normalized, raw=out, elapsed_s=round(time.monotonic() - t0, 4))
        return normalized
