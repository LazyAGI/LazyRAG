...
def build_report(session: AnalysisSession, synthesis: SynthesisResult | None) -> DiagnosisReport:
    ...
    return DiagnosisReport(report_id=rid, metadata={'created_at': timestamp.isoformat(), 'run_id': session.run_id, 'total_cases': len(session.case_step_features) or len(session.parsed_judge), 'pipeline': list(session.trace_meta.pipeline), 'eval_report_meta': session.eval_report_meta}, summary=s.summary, guidance=s.guidance, actions=[asdict(a) for a in s.actions], open_gaps=list(s.open_gaps), hypotheses=[asdict(h) for h in (world.hypotheses if world else [])], findings=[asdict(f) for f in (world.findings if world else [])], global_step_analysis=session.global_step_analysis, flow_analysis=jsonable(flow) if flow is not None else None, synthesizer_iterations=s.iterations)
def persist_report(session: AnalysisSession, report: DiagnosisReport) -> dict[str, Path | None]:
    ...
    bundle = write_report_bundle(session=session, payload=payload, markdown=md_text, base_name=f'reports/{report.report_id}')
