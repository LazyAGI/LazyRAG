...
class Conductor:
    ...
    def run(self) -> ConductorRunResult:
        total = 0
        self.session.telemetry.emit('conductor.started', max_iterations=self.cfg.max_iterations, max_concurrent=self.cfg.max_concurrent)
        for it in range(self.cfg.max_iterations):
            self._tick(it)
            decision = self._plan(it)
            actions = self._filter(decision.get('actions') or [])
            self.session.telemetry.emit('conductor.decision', iteration=it + 1, input=self._snapshot(it), output={'decision': decision, 'actions': actions})
            self._log.info('Conductor iter=%d done=%s actions_planned=%d actions_run=%d', it + 1, decision.get('done'), len(decision.get('actions') or []), len(actions))
    ...
    def _plan(self, iteration: int) -> dict[str, Any]:
        ...
        return invoke_structured(self.session, self._invoker, user, agent=CONDUCTOR_NAME, schema=SCHEMAS['conductor']) or {'actions': [], 'done': True}
    def _snapshot(self, iteration: int) -> dict[str, Any]:
        w = self.session.world_store.world
        return {'iteration': iteration + 1, 'max_iterations': self.cfg.max_iterations, 'hypotheses': [{'id': h.id, 'claim': h.claim, 'category': h.category, 'status': h.status, 'confidence': h.confidence, 'investigation_paths': list(h.investigation_paths)} for h in w.hypotheses], 'findings': [{'id': f.id, 'hypothesis_id': f.hypothesis_id, 'verdict': f.verdict, 'critic_status': f.critic_status, 'critic_notes': list(f.critic_notes)} for f in w.findings], 'open_questions': list(w.open_questions), 'budget_remaining': self.cfg.max_iterations - iteration}
    ...
    def _emit(self, iterations: int, converged: bool, total: int) -> None:
        self.session.telemetry.emit('agent_run', agent=CONDUCTOR_NAME, perspective=CONDUCTOR_NAME, iterations=iterations, converged=converged, total_actions=total)
