...
def run_tests(repo_root: Path, artifact_dir: Path, command: Sequence[str]=('bash', 'tests/run-all.sh'), *, on_proc: Callable[[subprocess.Popen], None] | None=None) -> TestOutcome:
    artifact_dir.mkdir(parents=True, exist_ok=True)
    command = _resolve_command(repo_root, command)
    log.info('running tests: cmd=%s cwd=%s', list(command), repo_root)
    proc = subprocess.Popen(list(command), cwd=str(repo_root), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if on_proc:
        on_proc(proc)
    ...
    return TestOutcome(passed=passed, returncode=proc.returncode, log_path=log_path, traceback_md_path=traceback_md_path, failed_tests=failed_tests)
