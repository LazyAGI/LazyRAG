...
_NO_CHANGES_FEEDBACK = '上一轮 opencode 未对任何 allowlist 中的文件做出修改。本轮必须实际改动至少一个允许文件，否则任务无法收尾。'
...
class ApplyOptions:
    max_rounds: int = 3
    test_command: tuple[str, ...] = field(default_factory=lambda : tuple(shlex.split(os.getenv('EVO_APPLY_TEST_CMD', 'bash tests/run-all.sh'))))
    instruction: str = '根据 report 完成代码修改'
    opencode_options: oc.OpencodeOptions = field(default_factory=lambda : oc.OpencodeOptions(model=os.getenv('EVO_OPENCODE_MODEL') or None, agent=os.getenv('EVO_OPENCODE_AGENT') or None, variant=os.getenv('EVO_OPENCODE_VARIANT') or None, timeout_s=int(os.getenv('EVO_OPENCODE_TIMEOUT_S', '600'))))
...
def _build_initial_checkpoint(apply_id: str, base_commit: str, branch_name: str, worktree: str) -> dict:
    return {'apply_id': apply_id, 'status': 'running', 'base_commit': base_commit, 'branch_name': branch_name, 'worktree': str(worktree), 'next_round': 1, 'prior_failure': '', 'rounds': [], 'final_commit': None, 'preview_ready': False}
...
def _build_modification_plan(actions: list[dict], chat_source: Path) -> list[dict]:
    return [{'id': str(a.get('id', '')), 'title': str(a.get('title', '')), 'rationale': str(a.get('rationale', '')), 'suggested_changes': _sanitize_path_text(str(a.get('suggested_changes', '')), chat_source), 'priority': str(a.get('priority', '')), 'files': [_rel_under_chat(str(a.get('code_map_target', '')), chat_source)]} for a in actions]
def _build_prompt(instruction: str, plan: list[dict], allow_lines: list[str], prior_failure: str) -> str:
    ...
    parts.append('')
    parts.append('所有文件读写都必须使用上述相对路径；不要访问或修改 worktree 外的绝对路径，也不要修改未列出的依赖实现文件。')
    parts.append('')
...
def _exhausted_error(rounds: list[RoundResult], max_rounds: int) -> dict:
    ...
    if code == 'OPENCODE_NO_CHANGES':
        return {'code': 'OPENCODE_NO_CHANGES', 'kind': 'permanent', 'message': f'opencode 在 {max_rounds} 轮内均未产生文件变更', 'details': {'rounds': max_rounds}}
    if code == 'APPLY_PATH_OUT_OF_ALLOWLIST':
        det = (last.error or {}).get('details') or {} if last else {}
        return {'code': 'APPLY_PATH_OUT_OF_ALLOWLIST', 'kind': 'transient', 'message': f'allowlist 越界在 {max_rounds} 轮内未解决', 'details': dict(det)}
    return {'code': 'MAX_ROUNDS_EXCEEDED', 'kind': 'transient', 'message': f'tests still failing after {max_rounds} round(s)', 'details': {}}
def _opencode_api_error(last_error: dict) -> dict:
    ...
    return {'code': 'OPENCODE_API_ERROR', 'kind': 'permanent' if status in {401, 403} else 'transient', 'message': str(message or details.get('name') or 'opencode API error'), 'details': details}
def execute_apply(*, apply_id: str, report: dict, config: EvoConfig, workspace: GitWorkspace, thread_id: str | None=None, options: ApplyOptions | None=None, cancel_token: Any | None=None, on_round: Callable[[RoundResult], None] | None=None, on_proc: Callable[[Any], None] | None=None, resume: bool=False) -> ApplyResult:
    ...
    if resume:
        ...
        if cp is not None and cp.get('preview_ready'):
            ...
            return ApplyResult(apply_id=apply_id, base_commit=cp.get('base_commit', ''), branch_name=cp.get('branch_name', ''), status='SUCCEEDED', rounds=round_data, final_commit=cp.get('final_commit'), diff_index_path=Path(preview_dir) / 'index.json' if preview_dir else None)
    ...
    binary = oc.preflight(options.opencode_options.binary, auth_dir=config.storage.opencode_dir, options=options.opencode_options)
    ...
    for i in range(start_round, options.max_rounds + 1):
        ...
        try:
            outcome = oc.run_opencode(prompt, cwd=worktree, artifact_dir=round_dir / 'opencode', binary=binary, options=options.opencode_options, on_proc=on_proc)
        ...
        if outcome.returncode != 0:
            rr.error = {'code': 'OPENCODE_RUN_FAILED', 'kind': 'transient', 'message': f'opencode exit={outcome.returncode}', 'details': {'last_error': outcome.last_error}}
            rr.finished_at = time.time()
            rounds.append(rr)
            _append_checkpoint_round(cp, rr)
            _write_checkpoint(cp_path, cp)
        ...
        if oob is not None:
            ...
            rr.error = {'code': 'APPLY_PATH_OUT_OF_ALLOWLIST', 'kind': 'transient', 'message': 'changes outside allowlist were reverted', 'details': {'paths': oob}}
            rr.finished_at = time.time()
            rounds.append(rr)
            _append_checkpoint_round(cp, rr)
        ...
        if sha is None:
            rr.test_passed = False
            rr.error = {'code': 'OPENCODE_NO_CHANGES', 'kind': 'permanent', 'message': 'opencode 本轮未修改任何允许文件', 'details': {}}
            rr.finished_at = time.time()
            rounds.append(rr)
            _append_checkpoint_round(cp, rr)
    ...
    if final_status == 'SUCCEEDED':
        ...
        diff_index_path = write_diff_map(workspace=workspace, apply_id=apply_id, worktree=worktree, base_commit=base_commit, out_dir=preview_dir)
    ...
    _write_checkpoint(cp_path, cp)
    return ApplyResult(apply_id=apply_id, base_commit=base_commit, branch_name=branch, status=final_status, rounds=rounds, final_commit=final_commit, error=final_error, diff_index_path=diff_index_path)
