...
def apply_model() -> OpencodeProviderConfig:
    return OpencodeProviderConfig(
        config['evo_code_provider'], config['evo_code_model'], config['evo_code_api_key'],
        config['evo_code_base_url'], config['evo_code_label'],
    )
...
OpencodeOutcome = namedtuple('OpencodeOutcome', 'returncode text_summary last_error events_path stdout_path stderr_path')
...
class OpencodeSession:
    def __init__(self, *, cwd: Path, binary: str, options: OpencodeOptions,
        ...
        self.model = f'{self.provider_config.provider}/{self.provider_config.model}' if self.provider_config else options.model
    ...
    def run(self, prompt: str, artifact_dir: Path) -> OpencodeOutcome:
        ...
        for flag, value in (('--model', self.model), ('--agent', self.options.agent), ('--variant', self.options.variant)):
            if value:
                cmd.extend([flag, value])
        cmd.append(prompt)
        log.info('opencode run: cwd=%s timeout_s=%d model=%s', self.cwd, self.options.timeout_s, self.model)
        proc = subprocess.Popen(cmd, cwd=str(self.cwd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, env=self.env)
        if self.on_proc:
            self.on_proc(proc)
        try:
...
def default_auth_dir() -> Path:
    return Path(data_dir) if (data_dir := config['evo_code_data_dir']) else Path.home() / '.local' / 'share' / 'opencode'
...
def preflight(binary: str | None, *, auth_dir: Path | None = None, options: OpencodeOptions | None = None) -> str:
    ...
    try:
        ...
        msg = 'opencode --version timed out' if isinstance(exc, subprocess.TimeoutExpired) else 'opencode binary not executable'
...
def _write_outcome(artifact_dir: Path, returncode: int, stdout: str, stderr: str) -> OpencodeOutcome:
    ...
    stdout_path, stderr_path, events_path = (artifact_dir / name for name in ('stdout.log', 'stderr.log', 'events.jsonl'))
