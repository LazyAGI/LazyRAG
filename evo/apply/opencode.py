...
def preflight(binary: str | None, *, auth_dir: Path | None=None, options: OpencodeOptions | None=None) -> str:
    ...
    has_provider_key = bool(cfg and cfg.api_key) or ((model or '').startswith('deepseek/') and bool(os.getenv('DEEPSEEK_API_KEY'))) or ((model or '').startswith('maas/') and bool(os.getenv('LAZYRAG_MAAS_API_KEY')))
...
def run_opencode(prompt: str, *, cwd: Path, artifact_dir: Path, binary: str, options: OpencodeOptions, on_proc: ProcSink | None=None) -> OpencodeOutcome:
    ...
    if (model or '').startswith('maas/'):
        _disable_proxy(env)
    log.info('opencode run: cwd=%s timeout_s=%d model=%s agent=%s variant=%s', cwd, options.timeout_s, model, options.agent, options.variant)
    ...
    try:
        ...
        raise ApplyError('OPENCODE_TIMEOUT', 'opencode run timed out', {'timeout_s': options.timeout_s, 'cwd': str(cwd)})
    finally:
        _cleanup_temp_config(temp_config)
        _cleanup_temp_home(temp_home)
    ...
    return OpencodeOutcome(returncode=proc.returncode, text_summary=text_summary, last_error=last_error, events_path=events_path, stdout_path=stdout_path, stderr_path=stderr_path)
...
def _ensure_project_provider_config(cwd: Path, model: str | None, provider_config: OpencodeProviderConfig | None=None) -> Path | None:
    model = model or ''
    if provider_config is not None:
        ...
        if path.exists():
            return None
        config = {'$schema': 'https://opencode.ai/config.json', 'provider': {provider: {'npm': '@ai-sdk/openai-compatible', 'name': provider_config.label or provider, 'options': {'baseURL': provider_config.base_url.rstrip('/'), 'apiKey': f'{{env:{_api_key_env(provider)}}}'}, 'models': {provider_config.model: {'name': provider_config.model}}}}}
    ...
    if model.startswith('maas/'):
        ...
        config = {'$schema': 'https://opencode.ai/config.json', 'provider': {'maas': {'npm': '@ai-sdk/openai-compatible', 'name': 'MAAS', 'options': {'baseURL': base_url.rstrip('/'), 'apiKey': '{env:LAZYRAG_MAAS_API_KEY}'}, 'models': {model_name: {'name': model_name}}}}}
        path.write_text(json.dumps(config, ensure_ascii=False, indent=2), encoding='utf-8')
        return path
    config = {'$schema': 'https://opencode.ai/config.json', 'provider': {'deepseek': {'npm': '@ai-sdk/openai-compatible', 'name': 'DeepSeek', 'options': {'baseURL': 'https://api.deepseek.com', 'apiKey': '{env:DEEPSEEK_API_KEY}'}, 'models': {'deepseek-chat': {'name': 'DeepSeek Chat'}, 'deepseek-v4-flash': {'name': 'DeepSeek V4 Flash'}, 'deepseek-v4-pro': {'name': 'DeepSeek V4 Pro'}}}}}
    path.write_text(json.dumps(config, ensure_ascii=False, indent=2), encoding='utf-8')
    return path
