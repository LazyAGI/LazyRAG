...
class ThreadDriver:
    ...
    def run_ops_async(self, thread_id: str, ops: list[dict[str, Any]], *,
                      source: str) -> dict:
        ws = ThreadWorkspace(self.jm.config.storage.base_dir, thread_id)
        ...
        t = threading.Thread(
            ...
            args=(thread_id, ops, source),
            daemon=True,
            name=f'evo-thread-driver-{thread_id}',
        )
        with self._guard:
    ...
    def retry(self, thread_id: str) -> dict:
        ...
        if not row:
            raise _store.StateError('NO_RESUMABLE_TASK',
                                    f'thread {thread_id} has no paused or transient failed task')
        ws = ThreadWorkspace(self.jm.config.storage.base_dir, thread_id)
    ...
    def _execute_op(self, thread_id: str, item: dict[str, Any], source: str,
                    elog: EventLog, ws: ThreadWorkspace,
                    sink: object | None = None) -> dict | None:
        op_name = item['op']
        ...
        for attempt in range(1, 4):
            ...
            if task and task.get('status') == 'failed_transient' and attempt < 3:
                ...
                if task and task.get('status') == 'failed_transient':
                    self._fail_thread(ws, elog, RuntimeError(
                        f'{op_name} failed with task status {task.get("status")}'))
                    return None
            if attempt == 3:
                self._fail_thread(ws, elog, RuntimeError(
                    f'{op_name} failed with task status {(task or {}).get("status")}'))
                return None
        return None

    def _wait_task(self, task_id: str, elog: EventLog,
                   ws: ThreadWorkspace) -> dict | None:
    ...
    def _advance_defaults(self, thread_id: str, task: dict,
                          elog: EventLog, ws: ThreadWorkspace) -> None:
        current = task
        while True:
    ...
    def _active_task(self, thread_id: str) -> dict | None:
        rows = [
            r for r in _thread_task_rows(self.jm.config.storage.base_dir, thread_id)
            if r.get('flow') in MAIN_FLOWS
            and r.get('status') in ('queued', 'running', 'stopping',
                                    'paused', 'failed_transient')
        ]
    ...
    def _latest_resumable(self, thread_id: str) -> dict | None:
        rows = [
            r for r in _thread_task_rows(self.jm.config.storage.base_dir, thread_id)
            if r.get('flow') in MAIN_FLOWS
            and r.get('status') in ('paused', 'failed_transient')
        ]
    ...
    def _fail_thread(self, ws: ThreadWorkspace, elog: EventLog, exc: Exception) -> None:
        self._write_runtime(ws, {'status': 'failed', 'last_error': str(exc)})
...
def _emit_flow_control(ws: ThreadWorkspace, row: dict | None, event: str) -> None:
    ...
    EventLog(ws.events_path).append_event(
        ...
        payload={'status': (row or {}).get('status')},
    )
...
def _next_default_op(task: dict, store: _store.FsStateStore,
                     ws: ThreadWorkspace) -> dict | None:
    flow = task.get('flow')
