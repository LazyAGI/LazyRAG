...
def build_results_router(*, base_dir: Path, store: _store.FsStateStore) -> APIRouter:
    ...
    def datasets(thread_id: str) -> list[dict]:
        ...
        for dataset_id in dataset_ids:
            ...
            out.append({'dataset_id': dataset_id, 'path': str(path), 'exists': path.is_file(), 'case_count': len(data.get('cases') or []), 'kb_id': data.get('kb_id')})
        return out
    ...
    def eval_reports(thread_id: str) -> list[dict]:
        ...
        for path in sorted((ws.dir / 'evals').glob('*.json')):
            data = _read_json(path) or {}
            out.append({'eval_id': path.stem, 'path': str(path), 'report_id': data.get('report_id'), 'total_cases': data.get('total_cases'), 'metrics': data.get('metrics') or data.get('summary')})
        return out
    ...
    def analysis_reports(thread_id: str) -> list[dict]:
        ...
        for row in _store.list_flow_tasks_by_thread(store, 'run', thread_id):
            ...
            if not report_id:
                continue
            json_path = _first_existing(reports_dir / f'{report_id}.json', Path(base_dir) / 'work' / 'reports' / f'{report_id}.json', Path(base_dir) / 'reports' / f'{report_id}.json')
            md_path = _first_existing(reports_dir / f'{report_id}.md', Path(base_dir) / 'work' / 'reports' / f'{report_id}.md', Path(base_dir) / 'reports' / f'{report_id}.md')
            out.append({'run_id': row['id'], 'report_id': report_id, 'json_path': str(json_path), 'md_path': str(md_path), 'json': _read_json(json_path), 'markdown': _read_text(md_path)})
        return out
    @router.get('/diffs')
    def diffs(thread_id: str) -> list[dict]:
        ...
        for row in _store.list_flow_tasks_by_thread(store, 'apply', thread_id):
            ...
            out.append({'apply_id': apply_id, 'status': row.get('status'), 'preview_path': str(preview) if preview.is_file() else None, 'preview': preview_data or None, 'files': _diff_files(preview_data)})
        return out
    @router.get('/abtests')
    def abtests(thread_id: str) -> list[dict]:
        ...
        for abtest_id in ws.load_artifacts().get('abtest_ids') or []:
            ...
            out.append({'abtest_id': abtest_id, 'summary': _read_json(d / 'summary.json'), 'decision': _read_json(d / 'decision.json'), 'markdown': _read_text(d / 'summary.md')})
        return out
    return router
...
def _preview_index_path(base_dir: Path, ws: ThreadWorkspace, apply_id: str) -> Path:
    ...
    return _first_existing(ws.dir / 'outputs' / preview_rel, Path(base_dir) / 'work' / preview_rel, ws.dir / 'outputs' / legacy_rel, Path(base_dir) / 'work' / legacy_rel)
def _diff_files(preview: dict) -> list[dict]:
    ...
    for item in preview.get('files') or []:
        ...
        out.append({'path': item.get('path'), 'change_kind': item.get('change_kind'), 'additions': item.get('additions'), 'deletions': item.get('deletions'), 'diff_path': str(path) if str(path) else None, 'filename': path.name if path.name else None, 'content': _read_text(path) if path.is_file() else None})
    return out
