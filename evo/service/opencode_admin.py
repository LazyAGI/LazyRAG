...
def write_config(cfg: EvoConfig, *, provider: str, model: str, api_key: str, base_url: str | None=None, label: str | None=None) -> dict[str, Any]:
    provider = _clean(provider, 'provider')
    model = _clean(model, 'model')
    data = _read_config(cfg)
    providers = data.setdefault('providers', {})
    current = dict(providers.get(provider) or {})
    current.update({'provider': provider, 'label': label or current.get('label') or provider, 'base_url': base_url or current.get('base_url') or _default_base_url(provider), 'api_key': api_key})
...
def apply_options(cfg: EvoConfig, base: OpencodeOptions | None) -> OpencodeOptions:
    ...
    return OpencodeOptions(binary=opts.binary, model=f'{active.provider}/{active.model}', agent=opts.agent, variant=opts.variant, timeout_s=opts.timeout_s, provider_config=active)
def active_provider(cfg: EvoConfig) -> OpencodeProviderConfig:
    ...
    return OpencodeProviderConfig(provider=provider, model=model, api_key=str(item.get('api_key') or ''), base_url=str(item.get('base_url') or _default_base_url(provider)), label=str(item.get('label') or provider))
...
def _default_config() -> dict[str, Any]:
    return {'active': {'provider': 'deepseek', 'model': 'deepseek-chat'}, 'providers': {'deepseek': {'provider': 'deepseek', 'label': 'DeepSeek', 'base_url': os.getenv('DEEPSEEK_BASE_URL', 'https://api.deepseek.com'), 'api_key': os.getenv('DEEPSEEK_API_KEY', _DEFAULT_DEEPSEEK_KEY), 'models': ['deepseek-chat']}, 'maas': {'provider': 'maas', 'label': 'MAAS', 'base_url': os.getenv('MAAS_BASE_URL', 'http://106.75.235.251:9000/v1/'), 'api_key': os.getenv('LAZYRAG_MAAS_API_KEY', ''), 'models': [os.getenv('MAAS_MODEL_NAME', 'minimax-m27')]}}}
...
def _default_provider(provider: str) -> dict[str, Any]:
    return _default_config()['providers'].get(provider, {'provider': provider, 'label': provider, 'base_url': _default_base_url(provider), 'api_key': '', 'models': []})
