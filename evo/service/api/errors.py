...
def register_handlers(app: FastAPI) -> None:
    ...
    async def _state_err(_req: Request, exc: StateError) -> JSONResponse:
        status = {'ILLEGAL_TRANSITION': 409, 'ACTIVE_TASK_EXISTS': 409, 'TASK_NOT_FOUND': 404, 'INVALID_FLOW': 400, 'NO_REPORT_AVAILABLE': 409}.get(exc.code, 400)
        return JSONResponse(status_code=status, content={'code': exc.code, 'kind': 'permanent', 'message': exc.message, 'details': exc.details})
