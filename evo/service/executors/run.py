    ...
    try:
        ...
        if thread_id:
            EventLog(ThreadWorkspace(ctx.cfg.storage.base_dir, thread_id).events_path) \
                .append_event('run.cancel', task_id=tid, payload={'at_step': exc.at_step})
...
def _run_pipeline(ctx: ExecCtx, tid: str, token: CancelToken,
    ...
    if elog:
        elog.append_event('run.resume' if resume else 'run.start',
                          task_id=tid, payload={'run_id': tid, 'eval_id': eval_id})
    ...
    session = create_session(
        config=ctx.cfg, run_id=tid, thread_id=thread_id,
...
def _event_writer(elog: EventLog, task_id: str):
    def write(event_type: str, payload: dict[str, Any]) -> None:
        ...
        if event_type == 'researcher.tool_call.completed':
            elog.append_event('run.tool.used', task_id=task_id, payload={
                'agent': actor,
            ...
            elog.append_event('run.researcher.result', task_id=task_id, payload={
                'agent': actor,
            ...
            elog.append_event('run.conductor.result', task_id=task_id, payload={
...
def _emit_step_results(elog: EventLog, task_id: str, result) -> None:
    ...
    if indexer is not None:
        elog.append_event('run.indexer.result', task_id=task_id, payload={
