...
def execute(ctx: ExecCtx, tid: str) -> None:
    ...
    try:
        ...
        if thread_id:
            EventLog(ThreadWorkspace(ctx.cfg.storage.base_dir, thread_id).events_path).append_event('run.cancel', task_id=tid, payload={'at_step': exc.at_step})
...
def _run_pipeline(ctx: ExecCtx, tid: str, token: CancelToken, *, resume: bool=False) -> None:
    ...
    if elog:
        elog.append_event('run.resume' if resume else 'run.start', task_id=tid, payload={'run_id': tid, 'eval_id': eval_id})
    ...
    session = create_session(config=ctx.cfg, run_id=tid, thread_id=thread_id, llm_provider=default_llm_provider(ctx.cfg), embed_provider=default_embed_provider(ctx.cfg))
    if elog:
    ...
    if report_path is not None:
        ...
        if elog:
            elog.append_event('run.finish', task_id=tid, payload={'run_id': tid, 'eval_id': eval_id, 'report_id': rid, 'report_path': str(report_path)})
...
def _event_writer(elog: EventLog, task_id: str):
    def write(event_type: str, payload: dict[str, Any]) -> None:
        ...
        if event_type == 'researcher.tool_call.completed':
            elog.append_event('run.tool.used', task_id=task_id, payload={'agent': actor, 'tool': payload.get('tool'), 'round': payload.get('round'), 'input_summary': _summary(payload.get('args')), 'output_summary': payload.get('summary') or _summary(payload.get('output')), 'ok': payload.get('ok'), 'elapsed_s': payload.get('elapsed_s')})
        elif event_type == 'researcher.reasoning_summary':
            elog.append_event('run.researcher.result', task_id=task_id, payload={'agent': actor, 'rounds': payload.get('rounds'), 'tool_calls': payload.get('tool_calls'), 'result_summary': _summary(payload.get('final_answer'), limit=1200)})
        elif event_type == 'conductor.decision':
            decision = payload.get('decision') or {}
            elog.append_event('run.conductor.result', task_id=task_id, payload={'iteration': payload.get('iteration'), 'actions': decision.get('actions') if isinstance(decision, dict) else None, 'done': decision.get('done') if isinstance(decision, dict) else None, 'summary': _summary(decision, limit=1200)})
    return write
def _emit_step_results(elog: EventLog, task_id: str, result) -> None:
    indexer = result.get('indexer')
    if indexer is not None:
        elog.append_event('run.indexer.result', task_id=task_id, payload={'status': _step_status(result, 'indexer'), 'summary': _summary(indexer, limit=1600), 'result': _compact(indexer)})
    conduct = result.get('conduct')
    if conduct is not None:
        elog.append_event('run.conductor.result', task_id=task_id, payload={'status': _step_status(result, 'conduct'), 'summary': _summary(conduct, limit=1200), 'result': _compact(conduct)})
