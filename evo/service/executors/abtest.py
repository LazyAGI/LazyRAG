...
def execute(ctx: ExecCtx, tid: str) -> None:
    ...
    inputs = AbtestInputs(abtest_id=tid, thread_id=thread_id, apply_id=payload['apply_id'], baseline_eval_id=payload['baseline_eval_id'], dataset_id=payload['dataset_id'], apply_worktree=Path(payload['apply_worktree']), candidate_chat_id=payload.get('candidate_chat_id'), target_chat_url=payload.get('target_chat_url'), eval_options=payload.get('eval_options') or {}, policy=ctx.abtest_policy.get(tid) or VerdictPolicy(**policy_data), candidate_env=_candidate_env(Path(payload['apply_worktree'])))
    try:
        result = execute_abtest(inputs=inputs, workspace=ws, log=elog, chat_runner=runner, chat_registry=ctx.chat_registry, cfg=ctx.cfg, llm_factory=lambda : get_automodel(ctx.cfg.model_config.llm_role), cancel=token.requested)
        ctx.update_payload(tid, {'verdict': result.verdict, 'candidate_chat_id': result.candidate_chat_id, 'new_eval_id': result.new_eval_id})
        from evo.runtime.fs import atomic_write_json
        abtest_checkpoint = {'abtest_id': tid, 'status': result.status, 'verdict': result.verdict, 'candidate_chat_id': result.candidate_chat_id, 'new_eval_id': result.new_eval_id}
