...
def execute(ctx: ExecCtx, tid: str) -> None:
    ...
    try:
        if elog:
            elog.append_event('dataset_gen.start', task_id=tid, payload={'dataset_id': eval_name, 'kb_id': kb_id, 'algo_id': algo_id, 'num_cases': num_cases})
        ds = KBClient.from_config(ctx.cfg)
        llm_factory = _resolve_llm_factory(ctx.cfg)
        (path, data) = run_generate_pipeline(kb_id=kb_id, algo_id=algo_id, eval_name=eval_name, dataset_source=ds, config=ctx.cfg, thread_id=thread_id, llm_factory=llm_factory, cancel=token.requested, num_cases=num_cases, on_progress=lambda current, total: elog.append_event('dataset_gen.progress', task_id=tid, payload={'current': current, 'total': total, 'dataset_id': eval_name}) if elog else None)
        ...
        if thread_id:
            ...
            elog.append_event('dataset_gen.finish', task_id=tid, payload={'dataset_id': eval_name, 'path': path, 'cases': data.get('total_nums')})
        ctx.on_success(tid)
