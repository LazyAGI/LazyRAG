...
class JobManager:
    def __init__(self, st: store.FsStateStore, config: EvoConfig, *, apply_opts: ApplyOptions | None=None, chat_runner: ChatRunner | None=None, chat_registry: ChatRegistry | None=None) -> None:
        self._store = st
        self._cfg = config
    ...
    def apply_commits_for_thread(self, thread_id: str) -> list[dict]:
        ...
        for row in rows:
            ...
            for r in rounds:
                ...
                if not sha:
                    continue
                commits.append({'round': r.get('round'), 'commit_sha': sha, 'test_passed': r.get('test_passed'), 'files_changed': r.get('files_changed')})
            out.append({'apply_id': aid, 'status': row.get('status'), 'thread_id': row.get('thread_id'), 'branch_name': row.get('branch_name'), 'base_commit': row.get('base_commit'), 'final_commit': row.get('final_commit'), 'commits': commits, 'rounds': rounds})
        return out
    def submit_run(self, *, thread_id: str | None=None, eval_id: str | None=None, badcase_limit: int | None=None, score_field: str | None=None) -> str:
    ...
    def submit_apply(self, *, report_id: str | None=None, thread_id: str | None=None) -> str:
        ...
        return tid
    def submit_eval(self, *, thread_id: str, eval_id: str | None=None, dataset_id: str | None=None, target_chat_url: str | None=None, options: dict | None=None) -> str:
        ...
        return tid
    def submit_abtest(self, *, thread_id: str, apply_id: str, baseline_eval_id: str, dataset_id: str, apply_worktree: Path | None=None, target_chat_url: str | None=None, candidate_chat_id: str | None=None, eval_options: dict | None=None, policy: VerdictPolicy | dict | None=None) -> str:
        ...
        payload = {'apply_id': apply_id, 'baseline_eval_id': baseline_eval_id, 'dataset_id': dataset_id, 'apply_worktree': str(worktree), 'eval_options': eval_options or {}, 'policy': verdict_policy.__dict__}
        ...
        self._spawn(tid, 'abtest')
        return tid
    def submit_dataset_gen(self, *, thread_id: str | None=None, kb_id: str, algo_id: str | None=None, eval_name: str | None=None, num_cases: int | None=None) -> str:
    ...
    def _make_ctx(self) -> ExecCtx:
        return ExecCtx(store=self._store, cfg=self._cfg, is_cancelled=self._is_cancelled, register_proc=self._registry.register_proc, chat_runner_factory=lambda : self._chat_runner, chat_registry=self._chat_registry, apply_opts=self._apply_opts, abtest_policy=self._registry._abtest_policy, on_stop=self._on_stop, on_failure=self._on_failure, on_success=self._on_success, pop_thread=self._registry.pop_thread, pop_procs=self._registry.pop_procs)
...
def _default_chat_runner(cfg: EvoConfig) -> ChatRunner:
    return SubprocessChatRunner(log_dir=cfg.storage.base_dir / 'state' / 'chats', health_path=os.getenv('EVO_CANDIDATE_CHAT_HEALTH_PATH', '/health'), startup_timeout_s=float(os.getenv('EVO_CANDIDATE_CHAT_STARTUP_TIMEOUT_S', '120')))
...
def _matching_apply(st: store.FsStateStore, thread_id: str | None, report_id: str) -> dict | None:
    ...
    for row in reversed(rows):
        if row.get('report_id') == report_id and row.get('status') in reusable:
            return row
    return None
def _matching_abtest(st: store.FsStateStore, thread_id: str, apply_id: str, baseline_eval_id: str, dataset_id: str, eval_options: dict | None=None) -> dict | None:
    rows = store.list_flow_tasks_by_thread(st, 'abtest', thread_id)
    reusable = {'queued', 'running', 'stopping', 'paused', 'failed_transient'}
    for row in reversed(rows):
        payload = row.get('payload') or {}
        if row.get('status') in reusable and payload.get('apply_id') == apply_id and (payload.get('baseline_eval_id') == baseline_eval_id) and (payload.get('dataset_id') == dataset_id) and ((payload.get('eval_options') or {}) == (eval_options or {})):
            return row
    return None
def _matching_resumable_abtest(st: store.FsStateStore, thread_id: str, apply_id: str, baseline_eval_id: str, dataset_id: str) -> dict | None:
    rows = store.list_flow_tasks_by_thread(st, 'abtest', thread_id)
    for row in reversed(rows):
...
def _require_apply_ready_for_abtest(st: store.FsStateStore, apply_row: dict) -> None:
    ...
    if status not in {'succeeded', 'accepted'}:
        raise store.StateError('APPLY_NOT_READY_FOR_ABTEST', f"apply {apply_row.get('id')} must finish before abtest", {'status': status})
    final_commit = apply_row.get('final_commit') or result.get('final_commit')
    if result.get('status') != 'SUCCEEDED' or not final_commit:
        raise store.StateError('APPLY_NOT_READY_FOR_ABTEST', f"apply {apply_row.get('id')} has no successful final commit", {'result_status': result.get('status'), 'final_commit': final_commit})
    rounds = store.list_rounds(st, apply_row['id'])
    if not rounds or rounds[-1].get('test_passed') != 1:
        raise store.StateError('APPLY_TESTS_NOT_PASSED', f"apply {apply_row.get('id')} final round did not pass tests", {'round_count': len(rounds)})
