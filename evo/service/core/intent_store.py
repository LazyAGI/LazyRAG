...
class IntentStore:
    ...
    def save(self, intent: Intent) -> None:
        ...
        data = {'intent_id': intent.intent_id, 'thread_id': intent.thread_id, 'user_message': intent.user_message, 'reply': intent.reply, 'suggested_ops_preview': [{'op': p.op, 'humanized': p.humanized, 'safety': p.safety, 'params_summary': p.params_summary} for p in intent.suggested_ops_preview], 'requires_confirm': intent.requires_confirm, 'thinking': intent.thinking, 'trace': intent.trace, 'created_at': intent.created_at, 'status': 'pending_confirm'}
    ...
    def transition(self, intent_id: str, action: str) -> dict:
        ...
        if current == 'expired':
            raise StateError('INTENT_EXPIRED', f'intent {intent_id} has expired (TTL={self._TTL_S}s)', {'status': 'expired'})
        if current in ('cancelled', 'materialized'):
            raise StateError('INTENT_ALREADY_FINAL', f'intent {intent_id} is already {current}', {'current_status': current, 'action': action})
        transitions = {'pending_confirm': {'confirm': 'confirmed', 'cancel': 'cancelled'}, 'confirmed': {'materialize': 'materialized', 'cancel': 'cancelled'}}
        allowed = transitions.get(current, {})
        if action not in allowed:
            raise StateError('INTENT_ILLEGAL_TRANSITION', f'intent {intent_id} is {current}, cannot {action}', {'current_status': current, 'action': action, 'allowed': list(allowed)})
