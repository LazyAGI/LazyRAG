    ...
    def execute(self, ops: list[Op], *, thread_id: str | None=None, idem_key: str | None=None) -> list[OpResult]:
        ...
        for op in ops:
            try:
                ...
                results.append(OpResult(op=op.op, status='rejected', error={'code': 'VALIDATION_ERROR', 'message': str(exc)}))
                continue
            args = dict(op.args)
            if thread_id:
            ...
            if handler is None:
                results.append(OpResult(op=op.op, status='unknown', error={'code': 'UNSUPPORTED_OP', 'message': f'{op.op} not implemented'}))
                continue
            try:
                ...
                results.append(OpResult(op=op.op, status='failed', error={'code': getattr(exc, 'code', 'EXEC_ERROR'), 'message': str(exc)}))
        return results
...
for _h in [_h_run_start, _h_run_stop, _h_run_continue, _h_run_cancel, _h_task_stop_active, _h_task_cancel_active, _h_task_continue_latest, _h_apply_start, _h_apply_stop, _h_apply_continue, _h_apply_cancel, _h_apply_accept, _h_apply_reject, _h_eval_run, _h_eval_fetch, _h_eval_cancel, _h_abtest_create, _h_abtest_stop, _h_abtest_continue, _h_abtest_cancel]:
...
def _validate_args(op: str, args: dict[str, Any]) -> dict[str, Any]:
    model_by_op = {'run.start': schemas.RunCreate, 'apply.start': schemas.ApplyCreate, 'dataset_gen.start': schemas.DatasetGenCreate, 'eval.run': schemas.EvalCreate, 'eval.fetch': schemas.EvalCreate, 'abtest.create': schemas.AbtestCreate}
...
def _resolve_latest_resumable_task(jm: JobManager, args: dict[str, Any]) -> str:
    ...
    best: dict | None = None
    for fl in flows:
        rows = store.list_flow_tasks_by_thread(jm.store, fl, thread_id) if thread_id else store.list_recent(jm.store, fl, 100)
        for row in rows:
