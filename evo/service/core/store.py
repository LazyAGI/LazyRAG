...
_ROUND_PATCH_FIELDS = frozenset({
    'phase', 'commit_sha', 'files_changed',
    'test_passed', 'error_json', 'finished_at',
})
...
def has_active(store: FsStateStore, flow: str, *,
               scope: str = 'global', thread_id: str | None = None) -> str | None:
...
def list_active(store: FsStateStore, flow: str, *,
                scope: str = 'global', thread_id: str | None = None) -> list[dict]:
    if scope not in ('global', 'thread'):
        raise ValueError("scope must be 'global' or 'thread'")
    terminals = set(terminal_for(flow))
    out: list[dict] = []
...
def transition_many(store: FsStateStore, ids: list[str], action: str,
                    **fields: Any) -> list[dict]:
    results: list[dict] = []
...
def create_task(store: FsStateStore, flow: str, *,
    ...
    with _flock(store._create_lock):
        active = has_active(store, flow, scope='thread' if thread_id else 'global',
                            thread_id=thread_id)
        if active:
...
def transition(store: FsStateStore, task_id: str, action: str,
               **fields: Any) -> dict:
...
def signals(store: FsStateStore, task_id: str) -> dict:
    ...
    return {'stop': bool(rec.get('request_stop')),
            'cancel': bool(rec.get('request_cancel'))}


def list_recent(store: FsStateStore, flow: str, limit: int = 50) -> list[dict]:
    out = [rec for path in store._all_task_files()
           if (rec := _read_task(path)) and rec.get('flow') == flow]
...
def append_round(store: FsStateStore, apply_id: str, round_idx: int,
                 *, phase: str = 'init') -> None:
    rec = {
        'apply_id': apply_id, 'round': round_idx, 'phase': phase,
        'commit_sha': None, 'files_changed': None,
        'test_passed': None, 'error_json': None,
        'started_at': time.time(), 'finished_at': None,
    }
    _atomic_write(store.round_path(apply_id, round_idx),
                  json.dumps(rec, ensure_ascii=False, indent=2))


def update_round(store: FsStateStore, apply_id: str, round_idx: int,
                 **fields: Any) -> None:
    if not fields:
        return
    ...
    with _flock(path):
        rec = _read_task(path)
        if rec is None:
            raise StateError('ROUND_NOT_FOUND',
                             f'apply {apply_id} round {round_idx} not found')
        for k, v in fields.items():
