...
_LIFECYCLE: dict[str, dict[str, str]] = {'queued': {'start': 'running', 'cancel': 'cancelled'}, 'running': {'stop': 'stopping', 'cancel': 'cancelled', 'finish': 'succeeded', 'fail_transient': 'failed_transient', 'fail_permanent': 'failed_permanent'}, 'stopping': {'ack': 'paused', 'cancel': 'cancelled'}, 'paused': {'continue': 'running', 'cancel': 'cancelled'}, 'failed_transient': {'continue': 'running', 'cancel': 'cancelled'}, 'failed_permanent': {}}
...
def _merge_lifecycle(*parts: dict[str, dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    return merged
_LEGAL: dict[str, dict[str, dict[str, str]]] = {'run': _merge_lifecycle(), 'apply': _merge_lifecycle(_APPLY_LIFECYCLE), 'eval': _merge_lifecycle(), 'abtest': _merge_lifecycle(), 'dataset_gen': _merge_lifecycle()}
TERMINAL: dict[str, set[str]] = {'dataset_gen': {'succeeded', 'failed_permanent', 'cancelled'}, 'eval': {'succeeded', 'failed_permanent', 'cancelled'}, 'run': {'succeeded', 'failed_permanent', 'cancelled'}, 'apply': {'succeeded', 'accepted', 'rejected', 'failed_permanent', 'cancelled'}, 'abtest': {'succeeded', 'failed_permanent', 'cancelled'}}
FLOWS: tuple[str, ...] = ('dataset_gen', 'eval', 'run', 'apply', 'abtest')
_PATCH_FIELDS = frozenset({'parent_run_id', 'report_id', 'base_commit', 'branch_name', 'final_commit', 'current_step', 'current_round', 'error_code', 'error_kind', 'thread_id', 'payload', 'dataset_id', 'source_eval_id'})
...
def next_status(flow: str, status: str, action: str) -> str:
    ...
    if action not in nexts:
        raise StateError('ILLEGAL_TRANSITION', f'flow={flow} status={status} cannot {action}', {'allowed': sorted(nexts)})
    return nexts[action]
...
def transition_many(store: FsStateStore, ids: list[str], action: str, **fields: Any) -> list[dict]:
    ...
    return results
def create_task(store: FsStateStore, flow: str, *, parent_run_id: str | None=None, report_id: str | None=None, thread_id: str | None=None, payload: dict | None=None) -> str:
    ...
    with _flock(store._create_lock):
        ...
        if active:
            raise StateError('ACTIVE_TASK_EXISTS', f'{flow} task {active} is not terminal', {'active_id': active, 'flow': flow})
        tid = _new_id(flow)
        now = time.time()
        rec = {'id': tid, 'flow': flow, 'status': 'queued', 'thread_id': thread_id, 'parent_run_id': parent_run_id, 'report_id': report_id, 'base_commit': None, 'branch_name': None, 'final_commit': None, 'current_step': None, 'current_round': None, 'request_stop': 0, 'request_cancel': 0, 'error_code': None, 'error_kind': None, 'payload': dict(payload or {}), 'created_at': now, 'updated_at': now, 'terminal_at': None}
...
def append_round(store: FsStateStore, apply_id: str, round_idx: int, *, phase: str='init') -> None:
    rec = {'apply_id': apply_id, 'round': round_idx, 'phase': phase, 'commit_sha': None, 'files_changed': None, 'test_passed': None, 'error_json': None, 'started_at': time.time(), 'finished_at': None}
