...
def _collect_ids_docids(items: Any, resolver: NodeResolver) -> tuple[list[str], list[str | None], list[str | None], list[int | None], list[str | None]]:
    ...
    for item in items:
        if isinstance(item, str):
            ...
            cid = item.get('id') or item.get('uid') or item.get('chunk_id') or item.get('node_id') or item.get('segment_id') or item.get('segement_id')
...
def _score_features(scores: np.ndarray) -> dict[str, float]:
    if not scores.size:
        return {}
    f = {'score_mean': float(np.mean(scores)), 'score_max': float(np.max(scores)), 'score_min': float(np.min(scores)), 'score_active_ratio': float((scores > 1e-06).mean())}
    if scores.size >= 2:
...
def _id_output_features(prefix: str, ids: list[str], gt: set[str]) -> dict[str, float]:
    ...
    f = {f'{prefix}_unique_count': float(unique.size), f'{prefix}_duplication_rate': float(1.0 - unique.size / out.size)}
    if not gt:
        return f
...
def _text_output_features(mod: ModuleOutput, judge: JudgeRecord, query: str, resolver: NodeResolver, embed_fn: EmbedFn | None) -> dict[str, float]:
    args = _get_args(mod.input)
    out_text = _extract_text(mod.output) or _resolve_chunks_text(mod.output if isinstance(mod.output, list) else [], resolver)
    ...
    return f
def features_for_step(mod: ModuleOutput, judge: JudgeRecord, resolver: NodeResolver, query: str='', embed_fn: EmbedFn | None=None) -> dict[str, float]:
    args = _get_args(mod.input)
    ...
    f: dict[str, float] = {'input_text_len': float(_text_len(mod.input)), 'output_text_len': float(_text_len(mod.output))}
    if query:
...
def build_case_step_features(judge: JudgeRecord, trace: TraceRecord, pipeline: list[str], resolver: NodeResolver, embed_fn: EmbedFn | None=None) -> dict[str, dict[str, float]]:
...
def flatten_case_features(step_feats: dict[str, dict[str, float]]) -> dict[str, float]:
    ...
    return flat
def build_step_matrix(all_case_feats: dict[str, dict[str, dict[str, float]]], step_key: str, target_ids: set[str] | None=None) -> tuple[list[str], list[str], np.ndarray]:
    (ids, rows) = ([], [])
...
def aggregate_global_step_analysis(all_case_feats: dict[str, dict[str, dict[str, float]]], all_judge: dict[str, JudgeRecord], pipeline: list[str]) -> dict[str, Any]:
    result: dict[str, Any] = {}
    ...
    for step_key in pipeline:
        ...
        for (metric, vals) in step_vectors.items():
            ...
            stats[metric] = {'mean': round(float(arr.mean()), 4), 'std': round(float(arr.std()), 4), 'min': round(float(arr.min()), 4), 'max': round(float(arr.max()), 4)}
