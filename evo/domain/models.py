...
# ---------------------------------------------------------------------------
# Parsing: legacy judge format
# ---------------------------------------------------------------------------

_JUDGE_REQUIRED = [
    "trace_id", "answer_correctness", "key", "hit_key", "reason",
    "context_recall", "doc_recall", "retrieved_file", "gt_file",
    "retrieved_text", "gt_text", "generated_answer", "gt_answer",
    "faithfulness", "human_verified",
]
...
# ---------------------------------------------------------------------------
# Parsing: trace formats (legacy flat + execution-tree)
# ---------------------------------------------------------------------------
...
def _walk_execution_tree(tree: dict[str, Any]) -> tuple[list[str], dict[str, ModuleOutput], list[dict[str, Any]]]:
    """Walk execution_tree by node_type. Returns (pipeline, modules, flow_skeleton)."""
    keys: list[str] = []
    ...
    # rename duplicates so all keys have _N suffix when name appears >1 time
    counts: dict[str, int] = {}
    for k in keys:
...
def parse_trace_file(raw: dict[str, Any]) -> tuple[TraceMeta, dict[str, TraceRecord], list[str]]:
    ...
    for key, val in raw.items():
        ...
        if ref_pipeline is None:
            ...
            if pipeline != ref_pipeline:
                warnings.append(f"[trace:{key}] pipeline differs from first case")

    meta = TraceMeta(
        flow_skeleton=ref_skeleton or [],
        pipeline=ref_pipeline or [],
    )
    return meta, traces, warnings


# ---------------------------------------------------------------------------
# Parsing: new eval report format
# ---------------------------------------------------------------------------
