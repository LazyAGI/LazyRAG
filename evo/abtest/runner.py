...
def execute_abtest(*,
    ...
    log.append_event('abtest.start', task_id=inputs.abtest_id, payload={
    ...
    try:
        for phase in PHASES:
            if cancel():
                ...
                log.append_event('abtest.finish', task_id=inputs.abtest_id, payload={
                    'abtest_id': inputs.abtest_id,
                    'status': 'cancelled',
        ...
        if ctx.candidate is not None and 'persist' not in state['completed']:
            chat_runner.stop(ctx.candidate.chat_id)
            chat_registry.purge(ctx.candidate.chat_id)
            _reset_candidate_checkpoint(state)
            _save_state(state_path, state)
...
def _reset_candidate_checkpoint(state: dict) -> None:
    ...
    state['completed'] = [p for p in completed
                          if p not in {'launch_chat', 'run_eval'}]
...
def _phase_run_eval(c: _Ctx) -> None:
    ...
    report = run_eval(
        ...
        on_progress=lambda current, total: c.log.append_event(
            'abtest.progress', task_id=c.inputs.abtest_id,
