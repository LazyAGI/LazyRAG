...
class AbtestResult:
    ...
    error: str | None = None
def execute_abtest(*, inputs: AbtestInputs, workspace: ThreadWorkspace, log: EventLog, chat_runner: ChatRunner, chat_registry: ChatRegistry, cfg, llm_factory=None, cancel: Callable[[], bool]=lambda : False) -> AbtestResult:
    ...
    log.append_event('abtest.start', task_id=inputs.abtest_id, payload={'abtest_id': inputs.abtest_id, 'apply_id': inputs.apply_id, 'baseline_eval_id': inputs.baseline_eval_id, 'dataset_id': inputs.dataset_id, 'candidate_chat_id': inputs.candidate_chat_id, 'target_chat_url': inputs.target_chat_url})
    try:
        for phase in PHASES:
            if cancel():
                ...
                log.append_event('abtest.finish', task_id=inputs.abtest_id, payload={'abtest_id': inputs.abtest_id, 'status': 'cancelled', 'candidate_chat_id': state.get('candidate_chat_id'), 'new_eval_id': state.get('new_eval_id')})
        ...
        try:
            ...
            if ctx.candidate is not None:
                chat_runner.stop(ctx.candidate.chat_id)
                chat_registry.purge(ctx.candidate.chat_id)
        log.append_event('abtest.finish', task_id=inputs.abtest_id, payload={'abtest_id': inputs.abtest_id, 'verdict': 'invalid', 'candidate_chat_id': state.get('candidate_chat_id'), 'candidate_chat_url': state.get('candidate_chat_url'), 'baseline_eval_id': inputs.baseline_eval_id, 'new_eval_id': state.get('new_eval_id'), 'summary_path': str(workspace.abtest_dir(inputs.abtest_id) / 'summary.json')})
        return AbtestResult('succeeded', 'invalid', state.get('summary'), state.get('candidate_chat_id'), state.get('new_eval_id'))
    ...
    if verdict == 'improved':
        ...
        pass
    log.append_event('abtest.finish', task_id=inputs.abtest_id, payload={'abtest_id': inputs.abtest_id, 'verdict': verdict, 'candidate_chat_id': state.get('candidate_chat_id'), 'candidate_chat_url': state.get('candidate_chat_url'), 'baseline_eval_id': inputs.baseline_eval_id, 'new_eval_id': state.get('new_eval_id'), 'summary_path': str(workspace.abtest_dir(inputs.abtest_id) / 'summary.json')})
...
def _phase_launch_chat(c: _Ctx) -> None:
    if c.candidate is None and c.inputs.target_chat_url:
        ...
        c.candidate = ChatInstance(chat_id=c.inputs.candidate_chat_id or f'chat-{c.inputs.abtest_id}', pid=None, port=_url_port(base_url), base_url=base_url, source_dir=c.inputs.apply_worktree, health_url=f'{base_url}/health', status='healthy', owner_thread_id=c.inputs.thread_id)
    ...
    if c.candidate is None or c.candidate.status != 'healthy':
        c.candidate = c.runner.launch(source_dir=c.inputs.apply_worktree, label=c.inputs.judge_label, env=c.inputs.candidate_env, owner_thread_id=c.inputs.thread_id)
        c.registry.register(c.candidate)
...
def _invalid_summary(exc: Exception) -> dict:
    return {'verdict': 'invalid', 'aligned_cases': 0, 'metrics': {}, 'missing_metrics': [], 'top_diff_cases': [], 'reasons': [f'candidate evaluation failed: {exc}'], 'error': str(exc)}
...
def _phase_run_eval(c: _Ctx) -> None:
    ...
    report = run_eval(dataset_id=c.inputs.dataset_id, target_chat_url=_chat_api_url(c.candidate.base_url), cfg=c.cfg, llm_factory=c.llm_factory, max_workers=c.inputs.eval_options.get('max_workers', 10), dataset_name=c.inputs.eval_options.get('dataset_name', ''), filters=c.inputs.eval_options.get('filters') or {}, on_progress=lambda current, total: c.log.append_event('abtest.progress', task_id=c.inputs.abtest_id, payload={'current': current, 'total': total, 'dataset_id': c.inputs.dataset_id}))
...
def _phase_persist(c: _Ctx) -> None:
    ...
    decision = {'verdict': summary.get('verdict'), 'candidate_chat_id': c.state.get('candidate_chat_id'), 'candidate_chat_url': c.state.get('candidate_chat_url'), 'baseline_eval_id': c.inputs.baseline_eval_id, 'new_eval_id': c.state.get('new_eval_id'), 'dataset_id': c.inputs.dataset_id, 'apply_id': c.inputs.apply_id}
...
_PHASES_FN: dict[str, Callable[[_Ctx], None]] = {'launch_chat': _phase_launch_chat, 'run_eval': _phase_run_eval, 'compare': _phase_compare, 'persist': _phase_persist}
...
def _summary_markdown(summary: dict, inputs: AbtestInputs) -> str:
    ...
    lines = [f'# abtest {inputs.abtest_id}', '', f'- baseline: `{inputs.baseline_eval_id}`', f'- dataset: `{inputs.dataset_id}`', f'- apply: `{inputs.apply_id}`', f"- verdict: **{summary.get('verdict')}**", f"- aligned cases: {summary.get('aligned_cases')}", '', '## metrics', '', '| metric | mean A | mean B | Δmean | win_rate B | sign p |', '| --- | --- | --- | --- | --- | --- |']
    for (m, info) in (summary.get('metrics') or {}).items():
        lines.append(f"| {m} | {info.get('mean_a')} | {info.get('mean_b')} | {info.get('delta_mean')} | {info.get('win_rate_b')} | {info.get('sign_p')} |")
