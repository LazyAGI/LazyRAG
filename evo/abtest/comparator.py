...
def _index_cases(report: dict) -> dict[str, dict]:
    return {k: case for case in (report.get('case_details') or [])
            if (k := _key(case))}


def _scores(cases: Iterable[dict], metric: str) -> list[float]:
    return [float(v) for c in cases
            if isinstance(v := c.get(metric), (int, float))]
...
_EMPTY_DIFF = {'mean_a': None, 'mean_b': None, 'delta_mean': None,
               'win_rate_b': None, 'sign_p': None, 'n': 0}
...
def compare_evals(report_a: dict, report_b: dict, *,
    ...
    for k in aligned:
        ...
        if isinstance(av, (int, float)) and isinstance(bv, (int, float)):
            deltas.append((k, round(float(bv) - float(av), 6),
                           float(av), float(bv)))
