...
def normalize_step(step: dict) -> dict:
    return {'name': step.get('name', ''), 'start_time': step.get('start_time', ''), 'end_time': step.get('end_time', ''), 'metadata': step.get('metadata', {}), 'inputs': step.get('inputs', {}), 'outputs': step.get('outputs', {})}
def normalize_trace(raw: dict) -> dict[str, Any]:
    steps = raw.get('steps', [])
    if isinstance(steps, list):
        steps = [normalize_step(s) for s in steps]
    trace = {'trace_id': raw.get('trace_id', ''), 'name': raw.get('name', ''), 'start_time': raw.get('start_time', ''), 'end_time': raw.get('end_time', ''), 'metadata': raw.get('metadata', {}), 'steps': steps}
...
def _fetch_trace_http(trace_id: str) -> dict[str, Any] | None:
    ...
    if not modules:
        modules = {data.get('name') or 'trace': {'input': data.get('input'), 'output': data.get('output'), 'scores': []}}
    return normalize_trace({'trace_id': data.get('id') or trace_id, 'name': data.get('name', ''), 'start_time': data.get('timestamp') or data.get('createdAt') or '', 'end_time': data.get('updatedAt') or '', 'metadata': data.get('metadata') or {}, 'steps': observations, 'query': (data.get('input') or {}).get('query', '') if isinstance(data.get('input'), dict) else '', 'modules': modules})
...
def _trace_from_eval_case(case: dict, *, error: str='') -> dict[str, Any]:
    ...
    modules = {'Retriever_KB': {'input': {'query': question}, 'output': {'sources': sources, 'contexts': case.get('retrieve_contexts') or [], 'doc_ids': case.get('retrieve_doc_ids') or [], 'chunk_ids': case.get('retrieve_chunk_ids') or []}, 'scores': []}, 'AnswerGenerator': {'input': {'query': question, 'contexts': case.get('retrieve_contexts') or []}, 'output': {'answer': answer, 'think': data.get('think')}, 'scores': []}}
    trace = {'trace_id': trace_id, 'name': 'eval.rag_response', 'query': question, 'metadata': {'source': 'eval_report', 'trace_fetch_error': error}, 'modules': modules, 'steps': []}
