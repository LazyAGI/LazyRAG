    ...
    for item in eval_results:
        ...
        if 'answer_correctness' in case:
            try:
                ...
                pass
    avg_score = round(sum(answer_correctness_list) / len(answer_correctness_list), 4) if answer_correctness_list else 0.0
    return {'report_id': str(uuid.uuid4()), 'dataset_id': eval_data.get('eval_name', ''), 'eval_name': eval_data.get('eval_name', ''), 'eval_set_id': eval_data.get('eval_set_id', ''), 'kb_id': eval_data.get('kb_id', ''), 'total_cases': total, 'avg_score': avg_score, 'created_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'case_details': case_details}
...
def build_full_eval_set(qa_result: list[dict], eval_name: str, kb_id: str, task_id: str='') -> dict[str, Any]:
    ...
    for item in qa_result:
        try:
            ...
            if not qa:
                continue
            case = {'case_id': str(uuid.uuid4()), 'reference_doc': qa.get('reference_doc', []), 'reference_context': qa.get('reference_context', []), 'is_deleted': False, 'question': qa.get('question', ''), 'question_type': qa.get('question_type', 1), 'key_points': qa.get('key_points', []), 'ground_truth': qa.get('ground_truth', ''), 'generate_reason': qa.get('generate_reason', ''), 'reference_chunk_ids': qa.get('reference_chunk_ids', []), 'reference_doc_ids': qa.get('reference_doc_ids', [])}
            cases.append(case)
        except Exception:
            continue
    return {'eval_set_id': str(uuid.uuid4()), 'eval_name': eval_name, 'kb_id': kb_id, 'task_id': task_id, 'create_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'total_nums': len(cases), 'cases': cases}
