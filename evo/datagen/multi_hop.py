...
def generate_multi_hop(ds: KBClient, kb_id: str, algo_id: str, *, max_questions: int=20, llm_factory=None, max_workers: int=8) -> list[dict]:
    idx = build_corpus_index(ds, kb_id, algo_id, max_workers=max_workers)
    return generate_multi_hop_from_chunks(idx.chunks, count=max_questions, max_workers=max_workers, llm_factory=llm_factory)
