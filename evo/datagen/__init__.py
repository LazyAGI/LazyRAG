...
from evo.datagen.writer import build_eval_report, build_full_eval_set, ensure_eval_dir, extract_json, load_report, save_eval_report, write_full_eval_set
...
class EvalDatasetEmptyError(RuntimeError):
    ...
    kind = 'permanent'
__all__ = ['run_generate_pipeline', 'run_eval', 'load_report', 'fetch_traces_for_report', 'generate_single_hop', 'create_evaluate_task', 'get_eval_queue', 'calculate_metrics', 'build_eval_report', 'build_full_eval_set', 'save_eval_report', 'write_full_eval_set', 'load_report', 'ensure_eval_dir', 'extract_json', 'safe_parse_qa_json', 'prompt_generate_single_hop', 'prompt_evaluate', 'KBClient', 'fetch_traces_for_report']
_TYPE_ORDER = ('single_hop', 'multi_hop', 'table', 'list')
_TYPE_TO_QUESTION_TYPE = {'single_hop': 1, 'multi_hop': 2, 'table': 4, 'list': 5}
def run_generate_pipeline(kb_id: str, algo_id: str, eval_name: str, *, dataset_source: KBClient, config: EvoConfig, thread_id: str | None=None, llm_factory=None, cancel=None, num_cases: int | None=None, on_progress=None) -> tuple[str, dict[str, Any]]:
    ...
    corpus = build_corpus_index(dataset_source, kb_id, algo_id, cache_dir=config.storage.work_dir / 'datagen_cache', docs=docs, max_workers=workers)
    _check_cancel(cancel)
    if not corpus.chunks:
        raise KBChunksEmptyError(f'no chunks found for kb_id={kb_id} algo_id={algo_id}; ensure document chunk API is populated or mount LAZYRAG_UPLOAD_HOST_DIR')
    if plan['single_hop'] > 0:
        add('single_hop', generate_single_hop_from_chunks(corpus.chunks, count=plan['single_hop'], max_workers=workers, llm_factory=llm_factory))
    _check_cancel(cancel)
    if plan['multi_hop'] > 0:
        add('multi_hop', generate_multi_hop_from_chunks(corpus.chunks, count=plan['multi_hop'], max_workers=workers, llm_factory=llm_factory))
    _check_cancel(cancel)
    if plan['table'] > 0:
        add('table', generate_table_questions(corpus.chunks, count=plan['table'], max_workers=workers, llm_factory=llm_factory))
    _check_cancel(cancel)
    if plan['list'] > 0:
        add('list', generate_list_questions(corpus.chunks, count=plan['list'], max_workers=workers, llm_factory=llm_factory))
    missing = target_count - len(result)
    if missing > 0:
        _log.info('dataset_gen shortfall=%s; filling with single-hop', missing)
        add('single_hop', generate_single_hop(dataset_source, kb_id, algo_id, count=missing, max_workers=workers, llm_factory=llm_factory))
    ...
    if target_count and len(cases) < target_count:
        raise DatasetGenerationEmptyError(f'dataset generation produced {len(cases)}/{target_count} valid cases for {eval_name}')
...
def run_eval(dataset_id: str, target_chat_url: str, *, cfg: EvoConfig, llm_factory=None, max_workers: int=10, dataset_name: str='', filters: dict[str, Any] | None=None, persist_report: bool=True, on_progress=None) -> dict[str, Any]:
    _log.info('start eval dataset_id=%s target=%s', dataset_id, target_chat_url)
    eval_data = get_eval_queue(dataset_id, dataset_name=dataset_name, base_dir=cfg.storage.base_dir, target_chat_url=target_chat_url, max_workers=max_workers, filters=filters or {}, on_progress=on_progress)
