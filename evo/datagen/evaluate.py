...
def evaluate_answer(question: str, ground_truth: str, rag_answer: str, key_points: list[str], retrieve_contexts: list[str], *, llm_factory=None) -> dict[str, Any]:
...
def _normalize_eval_result(data: dict[str, Any]) -> dict[str, Any]:
    return {'answer_correctness': _score01(data.get('answer_correctness')), 'is_correct': bool(data.get('is_correct')), 'reason': str(data.get('reason') or '')[:300], 'faithfulness': _score01(data.get('faithfulness'))}
...
def create_evaluate_task(eval_queue: list[dict], *, llm_factory=None, max_workers: int=10) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {executor.submit(evaluate_answer, item['question'], item['ground_truth'], item['rag_answer'], item.get('key_points', []), item.get('retrieve_contexts', []), llm_factory=llm_factory): item for item in eval_queue}
