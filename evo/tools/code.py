...
def list_code_map() -> ToolResult[dict[str, Any]]:
    ...
    for (path_str, desc) in cm.items():
        ...
        entries.append(info)
    return ToolResult.success('list_code_map', {'files': entries, 'total': len(entries), 'note': 'code_map = modifiable files only; for reading other files, use read_source_file (path) or resolve_import (symbol).'})
def _summ_subject_index(result: ToolResult[Any]) -> str:
    d = result.data or {}
    return f"entry={d.get('subject_entry')}; steps={list(d.get('step_to_source', {}).keys())}; symbols={list(d.get('symbol_hints', {}).keys())}"
...
def list_subject_index() -> ToolResult[dict[str, Any]]:
    ...
    return ToolResult.success('list_subject_index', {'subject_entry': str(si.subject_entry) if si.subject_entry else None, 'step_to_source': {k: asdict(v) for (k, v) in si.step_to_source.items()}, 'symbol_hints': dict(si.symbol_hints)})
@tool(tags=['code'])
def read_source_file(file_path: str, start_line: int=0, end_line: int=0) -> ToolResult[dict[str, Any]]:
    ...
    if len(numbered) > _MAX_FILE_CHARS:
        ...
        truncated = True
    return ToolResult.success('read_source_file', {'path': str(resolved), 'total_lines': total, 'range': f'{s + 1}-{min(e, total)}', 'truncated': truncated, 'content': numbered})
@tool(tags=['code'])
def parse_code_structure(file_path: str, symbol_name: str | None=None) -> ToolResult[dict[str, Any]]:
    ...
    if resolved.suffix == '.py':
        ...
        if symbol_name:
            return ToolResult.failure('parse_code_structure', ErrorCode.INVALID_ARGUMENT, f'symbol_name only supported for .py files; got {resolved.suffix}')
...
def _zoom_python_symbol(source: str, symbol_name: str, file_path: str) -> ToolResult[dict[str, Any]]:
    ...
    target = next((n for n in tree.body if isinstance(n, (ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef)) and n.name == symbol_name), None)
    if target is None:
        names = [n.name for n in tree.body if isinstance(n, (ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef))]
        return ToolResult.failure('parse_code_structure', ErrorCode.INVALID_ARGUMENT, f'symbol {symbol_name!r} not found; available: {names}')
    end_line = getattr(target, 'end_lineno', target.lineno)
    lines = source.splitlines()
    snippet = '\n'.join(lines[target.lineno - 1:end_line])
    payload: dict[str, Any] = {'path': file_path, 'symbol': symbol_name, 'kind': 'class' if isinstance(target, ast.ClassDef) else 'function', 'start_line': target.lineno, 'end_line': end_line, 'source': snippet}
    if isinstance(target, ast.ClassDef):
        payload['methods'] = [{'name': m.name, 'line': m.lineno, 'args': [a.arg for a in m.args.args]} for m in target.body if isinstance(m, (ast.FunctionDef, ast.AsyncFunctionDef))]
...
def _parse_python_ast(source: str) -> dict[str, Any]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef):
            methods = [{'name': m.name, 'args': [a.arg for a in m.args.args], 'line': m.lineno} for m in node.body if isinstance(m, (ast.FunctionDef, ast.AsyncFunctionDef))]
...
_DEFAULT_PARAM_PATTERN = re.compile('\\b(top_?k|top_?n|threshold|similarity_cut_off|chunk_size|temperature|max_tokens|score|max_retries|timeout|model|source|type)\\b\\s*[:=]', re.IGNORECASE)
def _scan_default_params(file_path: str, lines: list[str]) -> ToolResult[dict[str, Any]]:
    ...
    return ToolResult.success('extract_config_values', {'found': found, 'missing': [], 'file': file_path, 'scanned_with_default_vocabulary': True})
@tool(tags=['code'])
def search_code_pattern(pattern: str, file_paths: list[str] | None=None, scope: Literal['project', 'package', 'all']='project', package: str | None=None) -> ToolResult[dict[str, Any]]:
    if not pattern:
    ...
    if file_paths:
        ...
        if scope in ('package', 'all'):
            if not package:
                if scope == 'package':
                    return ToolResult.failure('search_code_pattern', ErrorCode.INVALID_ARGUMENT, "scope='package' requires package='<name>'")
            else:
    ...
    for p in targets:
        ...
        if len(matches) >= _MAX_SEARCH_MATCHES:
            break
    return ToolResult.success('search_code_pattern', {'matches': matches, 'total': len(matches), 'truncated': len(matches) >= _MAX_SEARCH_MATCHES, 'pattern': pattern, 'scope': scope, 'package': package, 'files_scanned': len(targets)})
