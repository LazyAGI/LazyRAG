...
def summarize_metrics(case_ids: list[str] | None=None) -> ToolResult[dict[str, Any]]:
    ...
    if case_ids is not None:
        if not isinstance(case_ids, list):
            return ToolResult.failure('summarize_metrics', ErrorCode.INVALID_ARGUMENT, 'case_ids must be a list or None.')
        if len(case_ids) == 0:
            case_ids = None
    ...
    for m in _METRICS:
        vs = sorted(values[m])
        if not vs:
            summary[m] = {'mean': None, 'median': None, 'min': None, 'max': None, 'p10': None, 'p90': None, 'count': 0, 'missing_count': missing[m]}
            continue
        summary[m] = {'mean': round(sum(vs) / len(vs), 4), 'median': round(percentile(vs, 50), 4), 'min': round(min(vs), 4), 'max': round(max(vs), 4), 'p10': round(percentile(vs, 10), 4), 'p90': round(percentile(vs, 90), 4), 'count': len(vs), 'missing_count': missing[m]}
    ...
    key_hit_stats = {'total_keys': total_keys, 'total_hit_keys': total_hits, 'avg_hit_rate': round(sum(hit_rates) / len(hit_rates), 4) if hit_rates else 0.0}
    return ToolResult.success('summarize_metrics', {'metrics': summary, 'key_hit_stats': key_hit_stats, 'total_cases': len(target_ids), 'filtered': case_ids is not None})
...
def summarize_step_metrics(step_key: str | None=None, case_ids: list[str] | None=None, metric: str | None=None) -> ToolResult[dict[str, Any]]:
    session = get_current_session()
    if session is None or not session.case_step_features:
        return ToolResult.failure('summarize_step_metrics', ErrorCode.DATA_NOT_LOADED, 'Step features not computed (run features step first).')
    if case_ids is not None:
        if not isinstance(case_ids, list) or not case_ids:
            return ToolResult.failure('summarize_step_metrics', ErrorCode.INVALID_ARGUMENT, 'case_ids must be a non-empty list or None.')
    pipeline = list(session.trace_meta.pipeline)
    if step_key is not None:
        if step_key not in pipeline:
            return ToolResult.failure('summarize_step_metrics', ErrorCode.INVALID_ARGUMENT, f'step_key {step_key!r} not in pipeline {pipeline}')
    ...
    for step in target_steps:
        ...
        for (m, vals) in vectors.items():
            vs = sorted(vals)
            per_metric[m] = {'mean': round(sum(vs) / len(vs), 4), 'std': round((sum(((x - sum(vs) / len(vs)) ** 2 for x in vs)) / len(vs)) ** 0.5, 4), 'min': round(vs[0], 4), 'max': round(vs[-1], 4), 'p10': round(percentile(vs, 10), 4), 'p90': round(percentile(vs, 90), 4), 'count': len(vs)}
    ...
    return ToolResult.success('summarize_step_metrics', {'pipeline': pipeline, 'scope': {'step_key': step_key, 'metric': metric, 'case_ids_subset': case_ids is not None, 'n_cases_examined': len(target_cases)}, 'by_step': out})
@tool(tags=['stats'])
def correlate_metrics(case_ids: list[str] | None=None) -> ToolResult[dict[str, Any]]:
    ...
    return ToolResult.success('correlate_metrics', {'method': 'pearson', 'matrix': matrix, 'sample_size': n, 'warnings': warnings})
