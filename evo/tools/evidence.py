...
def _summ_export(result: ToolResult[Any]) -> str:
    ...
    return f"case={d.get('dataset_id')} score={metrics.get('answer_correctness')} hit_keys={len(d.get('judge_texts', {}).get('hit_key', []))}/{len(d.get('judge_texts', {}).get('key_points', []))} steps={list(d.get('step_summaries', {}).keys())}"
...
def export_case_evidence(dataset_id: str) -> ToolResult[dict[str, Any]]:
    ...
    if judge is None:
        return ToolResult.failure('export_case_evidence', ErrorCode.CASE_NOT_FOUND, f'Dataset ID not found: {dataset_id}. Use list_bad_cases() / list_cases_ranked() to enumerate real IDs; examples: {session.sample_dataset_ids()}')
    trace = session.get_trace(judge.trace_id)
    pipeline = session.trace_meta.pipeline or (list(trace.modules.keys()) if trace else [])
    metrics = {'answer_correctness': judge.answer_correctness, 'context_recall': judge.context_recall, 'doc_recall': judge.doc_recall, 'faithfulness': judge.faithfulness, 'key_hit_rate': len(judge.hit_key) / max(len(judge.key), 1)}
    cap = 500
    texts = {'query': (trace.query if trace else '')[:cap], 'generated_answer': judge.generated_answer[:cap], 'gt_answer': judge.gt_answer[:cap], 'key_points': judge.key[:20], 'hit_key': judge.hit_key[:20], 'gt_file': judge.gt_file[:20], 'retrieved_file': judge.retrieved_file[:20]}
    ...
    return ToolResult.success('export_case_evidence', {'dataset_id': dataset_id, 'pipeline': pipeline, 'judge_metrics': metrics, 'judge_texts': texts, 'step_summaries': step_summaries, 'step_features': step_feats})
...
def list_cases_ranked(score_field: str='answer_correctness', order: str='asc', limit: int=10) -> ToolResult[dict[str, Any]]:
