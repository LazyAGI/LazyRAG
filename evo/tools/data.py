...
def inspect_step_for_case(dataset_id: str, step_key: str) -> ToolResult[dict[str, Any]]:
    if not dataset_id or not isinstance(dataset_id, str):
        return ToolResult.failure('inspect_step_for_case', ErrorCode.INVALID_ARGUMENT, 'dataset_id must be a non-empty string')
    if not step_key or not isinstance(step_key, str):
        return ToolResult.failure('inspect_step_for_case', ErrorCode.INVALID_ARGUMENT, 'step_key must be a non-empty string')
    ...
    try:
        ...
        return ToolResult.failure('inspect_step_for_case', ErrorCode.CASE_NOT_FOUND, f'Dataset ID not found: {dataset_id}. Use list_bad_cases() / list_cases_ranked() to enumerate real IDs; examples: {session.sample_dataset_ids()}')
    ...
    if module is None:
        return ToolResult.failure('inspect_step_for_case', ErrorCode.INVALID_ARGUMENT, f'step_key {step_key!r} not present in trace; available: {list(merged.trace.modules.keys())}')
    feats = session.case_step_features.get(dataset_id, {}).get(step_key, {})
    return ToolResult.success('inspect_step_for_case', {'dataset_id': dataset_id, 'step_key': step_key, 'input': module.input, 'output': module.output, 'step_features': feats, 'judge_score': merged.judge.answer_correctness, 'pipeline': list(session.trace_meta.pipeline)})
@tool(tags=['inspect'])
def list_bad_cases(threshold: float=0.6, score_field: str | None=None, limit: int=10, offset: int=0, sort: str='asc') -> ToolResult[dict[str, Any]]:
    session = get_current_session()
    if session is None or not session.parsed_judge:
    ...
    for (did, j) in session.iter_judge():
        ...
        all_cases.append({'dataset_id': did, 'score': score, 'trace_id': j.trace_id, 'query_preview': trace.query if trace else None})
    ...
    return ToolResult.success('list_bad_cases', {'total_count': len(all_cases), 'cases': page, 'next_offset': next_offset, 'histogram': buckets, 'threshold': threshold, 'score_field': metric})
@tool(tags=['inspect'])
def compare_cases(dataset_id1: str, dataset_id2: str) -> ToolResult[dict[str, Any]]:
    ...
    try:
        ...
        return ToolResult.failure('compare_cases', ErrorCode.CASE_NOT_FOUND, f'{exc}. Use list_bad_cases() / list_cases_ranked() to enumerate real IDs; examples: {session.sample_dataset_ids()}')
    ...
    for m in metrics_to_compare:
        ...
        if v1 is None or v2 is None:
            continue
        diff = v1 - v2
        metrics_diff[m] = {'case1': v1, 'case2': v2, 'diff': diff, 'better': 'case1' if diff > 0 else 'case2' if diff < 0 else 'equal'}
    ...
    pipeline_diff = {'case1_pipeline': ppl_meta, 'case2_pipeline': ppl2, 'length_diff': len(ppl_meta) - len(ppl2), 'common_modules': list(set(ppl_meta) & set(ppl2)), 'unique_to_case1': list(set(ppl_meta) - set(ppl2)), 'unique_to_case2': list(set(ppl2) - set(ppl_meta))}
    module_diff = {'case1_module_count': len(case1.trace.modules), 'case2_module_count': len(case2.trace.modules), 'common_module_names': list(set(case1.trace.modules) & set(case2.trace.modules))}
    hints: list[str] = []
    ac = metrics_diff.get('answer_correctness')
    if ac and ac['diff']:
        hints.append(f"{('case1' if ac['better'] == 'case1' else 'case2')} has higher correctness; examine its retrieval and generation pipeline for transferable patterns.")
    ...
    return ToolResult.success('compare_cases', {'dataset_id1': dataset_id1, 'dataset_id2': dataset_id2, 'metrics_diff': metrics_diff, 'pipeline_diff': pipeline_diff, 'module_diff': module_diff, 'hypothesis_hints': hints})
@tool(tags=['inspect'])
