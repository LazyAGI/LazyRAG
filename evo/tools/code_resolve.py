...
def resolve_import(symbol: str) -> ToolResult[dict[str, Any]]:
    ...
    try:
        ...
        line = 1
    kind = 'module' if inspect.ismodule(src_obj) else 'class' if inspect.isclass(src_obj) else 'function' if inspect.isfunction(src_obj) or inspect.ismethod(src_obj) else 'attribute'
    sig = ''
    ...
    return ToolResult.success('resolve_import', {'symbol': symbol, 'kind': kind, 'file': file_path, 'line': int(line), 'signature': sig, 'doc_excerpt': doc, 'readable': readable, 'scope_reason': why})
