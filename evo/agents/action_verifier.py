...
def _build_user(action: VerifiedAction, evidence: list[dict[str, Any]]) -> str:
    view = {'id': action.id, 'title': action.title, 'rationale': action.rationale, 'suggested_changes': action.suggested_changes, 'expected_impact_metric': action.expected_impact_metric, 'expected_direction': action.expected_direction, 'confidence': action.confidence, 'evidence_handles': list(action.evidence_handles)}
...
def run_action_verifier(session: AnalysisSession, action: VerifiedAction, *, llm: Any | None=None) -> VerifiedAction:
    ...
    parsed = invoke_structured(session, invoker, user, agent=f'{VERIFIER_NAME}:{action.id}', schema=SCHEMAS['action_verifier'])
    ...
    session.telemetry.emit('agent_run', agent=f'{VERIFIER_NAME}:{action.id}', perspective=VERIFIER_NAME, action_id=action.id, validity_score=action.validity_score, supporting=len(action.supporting_evidence), contradicting=len(action.contradicting_evidence), elapsed_s=round(time.monotonic() - t_start, 4))
    return action
def verify_actions(session: AnalysisSession, actions: list[VerifiedAction], *, max_workers: int=4) -> list[VerifiedAction]:
    if not actions:
        return actions
