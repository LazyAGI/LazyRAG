...
def _build_user(finding: Finding, evidence: list[dict[str, Any]]) -> str:
    finding_view = {'id': finding.id, 'claim': finding.claim, 'verdict': finding.verdict, 'confidence': finding.confidence, 'suggested_action': finding.suggested_action, 'evidence_handles': list(finding.evidence_handles)}
...
def run_critic(session: AnalysisSession, finding: Finding, *, llm: Any | None=None) -> CriticVerdict:
    ...
    verdict = CriticVerdict(finding_id=finding.id, status=status, challenges=list(parsed.get('challenges', []) or []), approved_confidence=coerce_confidence(approved_conf) if approved_conf is not None else None)
    if session.world_store is not None:
        session.world_store.update(lambda w: _record(w, verdict))
    session.telemetry.emit('agent_run', agent=f'{CRITIC_NAME}:{finding.id}', perspective=CRITIC_NAME, finding_id=finding.id, status=verdict.status, challenges=len(verdict.challenges), elapsed_s=round(time.monotonic() - t_start, 4))
    return verdict
