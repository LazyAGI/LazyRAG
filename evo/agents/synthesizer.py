...
def run_synthesizer(session: AnalysisSession, *, llm: Any | None=None) -> SynthesisResult:
    ...
    result = SynthesisResult(summary=str(parsed.get('summary', '')), guidance=str(parsed.get('guidance', '')), actions=actions, open_gaps=[g for g in (_format_gap(g) for g in parsed.get('open_gaps', []) or []) if g], iterations=iterations)
    session.telemetry.emit('agent_run', agent=SYNTHESIZER_NAME, perspective=SYNTHESIZER_NAME, action_count=len(result.actions), open_gaps=len(result.open_gaps), iterations=result.iterations, elapsed_s=round(time.monotonic() - t_start, 4))
    return result
...
def _world_summary(session: AnalysisSession, iteration: int) -> dict[str, Any]:
    ...
    if w is None:
        return {'iteration': iteration, 'hypotheses': [], 'findings': [], 'open_questions': [], 'code_context': code_ctx}
    by_cat: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for h in w.hypotheses:
        by_cat[h.category or 'uncategorized'].append({'id': h.id, 'claim': h.claim, 'status': h.status, 'confidence': h.confidence, 'source': h.source})
    findings = [{'id': f.id, 'hypothesis_id': f.hypothesis_id, 'claim': f.claim, 'verdict': f.verdict, 'confidence': f.confidence, 'critic_status': f.critic_status, 'critic_notes': list(f.critic_notes), 'evidence_handles': list(f.evidence_handles), 'suggested_action': f.suggested_action} for f in w.findings if f.verdict in ('confirmed', 'inconclusive')]
    return {'iteration': iteration + 1, 'is_final_round': iteration + 1 >= 2, 'code_context': code_ctx, 'hypotheses_by_category': dict(by_cat), 'findings': findings, 'open_questions': list(w.open_questions)}
def _format_gap(g: Any) -> str:
    ...
    return str(g).strip()
_PRIORITY_ALIAS = {'high': 'P0', 'p0': 'P0', 'critical': 'P0', 'urgent': 'P0', 'medium': 'P1', 'med': 'P1', 'p1': 'P1', 'normal': 'P1', 'low': 'P2', 'p2': 'P2', 'minor': 'P2', 'optional': 'P2'}
_DIRECTION_ALIAS = {'up': '+', 'increase': '+', 'higher': '+', 'positive': '+', 'down': '-', 'decrease': '-', 'lower': '-', 'negative': '-'}
...
def _to_action(raw: Any, session: AnalysisSession) -> VerifiedAction | None:
    ...
    try:
        ...
        line = 0
    return VerifiedAction(id=aid, finding_id=str(raw.get('finding_id', '')), hypothesis_id=str(raw.get('hypothesis_id', '')), hypothesis_category=str(raw.get('hypothesis_category', '')), title=title, rationale=rationale, suggested_changes=suggested, priority=_normalize_priority(raw.get('priority')), expected_impact_metric=str(raw.get('expected_impact_metric', '')), expected_direction=_normalize_direction(raw.get('expected_direction')), confidence=max(0.0, min(1.0, confidence)), evidence_handles=[str(h) for h in raw.get('evidence_handles', []) or []], code_map_target=target, target_step=str(raw.get('target_step', '')), target_line=line)
