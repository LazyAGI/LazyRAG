...
def _build_input(session: AnalysisSession) -> dict[str, Any]:
    ...
    return {'pipeline': list(session.trace_meta.pipeline), 'total_cases': len(session.parsed_judge), 'avg_judge': _avg_judge(session), 'step_metrics': {step: stats.get('stats', stats) for (step, stats) in session.global_step_analysis.items()}, 'metric_directions': {m: d for (m, d) in METRIC_DIRECTION.items() if any((m in stats.get('stats', stats) for stats in session.global_step_analysis.values() if isinstance(stats, dict))) or m in _JUDGE_METRICS}, 'clusters': [{'id': cs.cluster_id, 'size': cs.size, 'score_stats': cs.score_stats, 'top_features': dict(list(cs.top_feature_deltas.items())[:8])} for cs in (cg.cluster_summaries if cg else [])], 'flow': {'transitions': [{'from': t.from_step, 'to': t.to_step, 'type': t.type, 'entropy_change': t.entropy_change, 'nmi': t.nmi} for t in (fa.transition_analysis if fa else [])], 'critical_steps': list(fa.critical_steps) if fa else []}}
...
def _append_hypotheses(world: WorldModel, hyps: list[Any]) -> None:
    ...
    for raw in hyps:
        ...
        if not isinstance(paths, list):
            paths = [paths]
        world.hypotheses.append(Hypothesis(id=hid, claim=str(h.get('claim', '')), category=str(h.get('category', '')), status='proposed', confidence=coerce_confidence(h.get('confidence')), evidence_handles=[], investigation_paths=[str(p) for p in paths], source=INDEXER_NAME))
def run_indexer(session: AnalysisSession, *, llm: Any | None=None, user_feedback: str | None=None) -> dict[str, Any]:
    ...
    parsed = invoke_structured(session, invoker, json.dumps(payload, ensure_ascii=False, indent=2), agent=INDEXER_NAME, schema=SCHEMAS['indexer'], cache_key=cache_key)
    ...
    session.telemetry.emit('agent_run', agent=INDEXER_NAME, perspective=INDEXER_NAME, hypothesis_count=len(hypotheses), open_questions=len(open_questions), elapsed_s=round(time.monotonic() - t_start, 4))
    return parsed
