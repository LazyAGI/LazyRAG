...
class MemoryCurator:
    ...
    def update(self, working_memory: str, *, tool: str, args_brief: str, summary: str, handle: str | None, ok: bool) -> str:
        user = f"当前 working_memory:\n{working_memory or '(empty)'}\n\n最新一次工具调用:\n- tool: {tool}\n- args: {args_brief or '(none)'}\n- summary: {summary}\n- handle: {handle or '(none)'}\n- ok: {str(ok).lower()}\n"
        raw = self.session.llm.call(producer=lambda : self.invoker.invoke(user), cache_key=None, use_cache=False, agent=f'{CURATOR_NAME}:{self.agent}')
        text = (raw or '').strip()
        if not text:
