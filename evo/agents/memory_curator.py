...
class MemoryCurator:
    def __init__(self, session: AnalysisSession, agent: str,
                 llm: Any | None = None) -> None:
        self.session = session
        self.agent = agent
        self.invoker = LLMInvoker(
            session=session, system_prompt=load_prompt("memory_curator"), llm=llm,
        )

    def update(self, working_memory: str, *, tool: str, args_brief: str,
               summary: str, handle: str | None, ok: bool) -> str:
        user = (
