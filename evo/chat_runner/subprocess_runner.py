...
class SubprocessChatRunner:
    def __init__(self, *, log_dir: Path, command: list[str] | None=None, health_path: str='/healthz', startup_timeout_s: float=30.0, stop_timeout_s: float=10.0) -> None:
    ...
    def launch(self, *, source_dir: Path, label: str, env: dict | None=None, owner_thread_id: str | None=None) -> ChatInstance:
        ...
        instance = ChatInstance(chat_id=chat_id, pid=proc.pid, port=port, base_url=base_url, source_dir=Path(source_dir), health_url=f'{base_url}{self.health_path}', status='starting', owner_thread_id=owner_thread_id)
