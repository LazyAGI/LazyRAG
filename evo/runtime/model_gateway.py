...
class ModelGateway(Generic[T]):
    def __init__(self, cfg: ModelGovernanceConfig, *, name: str='model', logger: logging.Logger | None=None, on_event: Callable[..., None] | None=None) -> None:
        self._cfg = cfg
    ...
    def call(self, producer: Callable[[], T], *, cache_key: str | None=None, use_cache: bool | None=None, agent: str='') -> T | None:
        if self._disabled:
            self._on_event('llm_call', gateway=self._name, agent=agent, ok=False, disabled=True, attempts=0, cache_hit=False, elapsed_s=0.0)
            return None
        ...
        if use_cache and cache_key:
            ...
            if hit is not None:
                ...
                self._on_event('llm_call', gateway=self._name, agent=agent, ok=True, cache_hit=True, attempts=0, elapsed_s=0.0)
                return hit
        ...
        for attempt in range(1, self._cfg.max_retries + 1):
            try:
                ...
                self._on_event('llm_call', gateway=self._name, agent=agent, ok=True, cache_hit=False, attempts=attempt, elapsed_s=round(time.monotonic() - t_start, 4))
                return out
        ...
        self._on_event('llm_call', gateway=self._name, agent=agent, ok=False, cache_hit=False, attempts=self._cfg.max_retries, elapsed_s=round(time.monotonic() - t_start, 4), error=type(last_exc).__name__)
