...
_DEFAULT_EXCLUDES = ('**/__pycache__/**', '**/.git/**', '**/.venv/**', '**/node_modules/**', '**/build/**', '**/dist/**')
@dataclass(frozen=True)
class ReadScope:
...
def code_context_dict(ca: 'CodeAccessConfig') -> dict[str, Any]:
    si = ca.subject_index
    return {'code_map_files': list(ca.code_map.keys()), 'subject_entry': str(si.subject_entry) if si.subject_entry else None, 'step_to_source': {k: {'file': v.file, 'line': v.line, 'symbol': v.symbol, 'init_args': v.init_args} for (k, v) in si.step_to_source.items()}, 'symbol_hints': dict(si.symbol_hints)}
...
def load_code_access(path: Path | None) -> CodeAccessConfig:
    ...
    step_to_source = {str(k): StepSource(file=str(v.get('file', '')), line=int(v.get('line', 0) or 0), symbol=str(v.get('symbol', '')), init_args=dict(v.get('init_args') or {})) for (k, v) in (si_raw.get('step_to_source') or {}).items() if isinstance(v, dict)}
