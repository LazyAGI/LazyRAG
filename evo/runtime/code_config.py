...
def code_context_dict(ca: 'CodeAccessConfig') -> dict[str, Any]:
    ...
    return {
        ...
        'step_to_source': {
            k: {'file': v.file, 'line': v.line, 'symbol': v.symbol,
...
def load_code_access(path: Path | None) -> CodeAccessConfig:
    ...
    subject_entry = (
        _resolve(si_raw['subject_entry'], base)
        if si_raw.get('subject_entry') else None
    )
    step_to_source = {
        str(k): StepSource(
            file=str(v.get('file', '')), line=int(v.get('line', 0) or 0),
