...
def _session() -> requests.Session:
    ...
    return s
def _base_url() -> str:
    return (os.getenv('EVO_CHUNK_BASE_URL') or os.getenv('EVO_KB_BASE_URL') or os.getenv('LAZYRAG_DOCUMENT_SERVER_URL') or '').rstrip('/')
def _candidate_kb_ids() -> list[str]:
    raw = ','.join((v for v in (os.getenv('EVO_NODE_KB_IDS', ''), os.getenv('EVO_NODE_KB_ID', ''), os.getenv('EVO_KB_ID', ''), os.getenv('LAZYRAG_KB_ID', '')) if v))
    return [x.strip() for x in raw.split(',') if x.strip()]
def _try_direct_node(base: str, node_id: str) -> NodeInfo | None:
    for (path, params) in (('/v1/nodes', {'node_id': node_id}), ('/v1/chunks', {'chunk_id': node_id}), ('/v1/chunks', {'uid': node_id})):
        try:
...
def _find_doc(base: str, kb_id: str, node_id: str) -> NodeInfo | None:
    for page in range(1, _max_pages() + 1):
        try:
            r = _session().get(f'{base}/v1/docs', params={'kb_id': kb_id, 'algo_id': _algo_id(), 'page': page, 'page_size': 100}, timeout=_timeout())
            if not r.ok:
                return None
            items = _items(r.json())
            if not items:
            ...
            for item in items:
                ...
                if doc_id == node_id:
                    return NodeInfo(id=node_id, docid=doc_id, kb_id=kb_id, file_name=str(doc.get('filename') or doc.get('file_name') or doc.get('name') or ''), text=str(doc.get('content') or doc.get('text') or ''))
        except Exception as exc:
...
def _find_chunk(base: str, kb_id: str, node_id: str) -> NodeInfo | None:
    for doc in _iter_docs(base, kb_id):
        ...
        for group in ('block', 'line'):
            for page in range(1, _max_pages() + 1):
                try:
                    r = _session().get(f'{base}/v1/chunks', params={'kb_id': kb_id, 'doc_id': doc_id, 'group': group, 'algo_id': _algo_id(), 'page': page, 'page_size': 100}, timeout=_timeout())
                    if not r.ok:
                        break
                    items = _items(r.json())
                    if not items:
...
def _iter_docs(base: str, kb_id: str) -> list[dict[str, Any]]:
    ...
    for page in range(1, _max_pages() + 1):
        try:
            ...
            break
    return out
def _node_from_chunk(item: Any, *, fallback_doc: dict[str, Any] | None=None, kb_id: str | None=None, group: str | None=None) -> NodeInfo | None:
    ...
    file_name = meta.get('file_name') or meta.get('filename') or item.get('file_name') or item.get('filename') or (fallback_doc or {}).get('filename')
    return NodeInfo(id=str(node_id), docid=str(doc_id) if doc_id else '', kb_id=str(item.get('kb_id') or kb_id or ''), file_name=str(file_name or ''), text=str(item.get('content') or item.get('text') or ''), group=str(item.get('group') or group or ''), page=_int_or_none(item.get('page') or meta.get('page')), index=_int_or_none(item.get('index') or meta.get('index')), number=_int_or_none(item.get('number') or meta.get('number')), bbox=item.get('bbox') or meta.get('bbox') or [])
