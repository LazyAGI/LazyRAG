...
def http_get_node(node_id: str) -> NodeInfo | None:
    """Resolve a trace node/chunk/doc id from the document service.

    Trace backends often serialize DocNode outputs as ids or lossy strings.  The
    resolver deliberately avoids importing chat internals and uses the same
    document/chunk HTTP APIs as dataset generation.
    """
...
def _session() -> requests.Session:
    s = requests.Session()
    # Internal Docker service names must not be routed through the outbound proxy.
    s.trust_env = False
    return s
            ...
            for page in range(1, _max_pages() + 1):
                try:
                    r = _session().get(
                        f'{base}/v1/chunks',
                        params={
                            'kb_id': kb_id, 'doc_id': doc_id, 'group': group,
                            'algo_id': _algo_id(), 'page': page, 'page_size': 100,
                        },
                        timeout=_timeout(),
                    )
                    if not r.ok:
...
def _node_from_chunk(
    item: Any, *, fallback_doc: dict[str, Any] | None = None,
    kb_id: str | None = None, group: str | None = None,
) -> NodeInfo | None:
    ...
    file_name = (
        meta.get('file_name') or meta.get('filename') or item.get('file_name')
        or item.get('filename') or (fallback_doc or {}).get('filename')
    )
    return NodeInfo(
        id=str(node_id),
