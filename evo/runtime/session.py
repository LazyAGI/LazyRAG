...
from evo.domain import ClusteringResult, FlowAnalysisResult, JudgeRecord, MergedCaseView, NodeInfo, NodeResolver, PerStepClusteringResult, TraceMeta, TraceRecord
...
class AnalysisSession:
    ...
    def score_lookup(self, score_field: str) -> Callable[[str], float | None]:
        ...
        return _lookup
    def set_parsed_corpus(self, judges: dict[str, JudgeRecord], traces: dict[str, TraceRecord], trace_meta: TraceMeta, *, warnings: list[str] | None=None, eval_report_meta: dict[str, Any] | None=None) -> None:
    ...
    def set_step_features(self, case_features: dict[str, dict[str, dict[str, float]]], global_features: dict[str, Any]) -> None:
...
def create_session(config: EvoConfig | None=None, *, run_id: str | None=None, thread_id: str | None=None, llm_provider: LLMProvider | None=None, embed_provider: EmbedProvider | None=None, node_resolver: NodeResolver=http_get_node) -> AnalysisSession:
    if config is None:
    ...
    def write_event(kind: str, payload: dict[str, Any]) -> None:
        ...
        passthrough = ('handle.', 'world_model.', 'researcher.', 'llm.', 'conductor.', 'artifact.', 'task.', 'op.', 'schema.')
        ...
        if event_output is None and metadata is not None:
            ...
            metadata = None
        event_sink.emit(event_kind, actor=actor, task_id=run_id, input=event_input, output=event_output, metadata=metadata)
    telemetry_path = None if event_sink is not None else run_dir / 'telemetry.jsonl'
    handle_store = HandleStore(None if event_sink is not None else run_dir / 'handles.jsonl', event_writer=write_event if event_sink is not None else None)
    world_store = WorldModelStore(None if event_sink is not None else run_dir / 'world_model.json', run_id=run_id, event_writer=write_event if event_sink is not None else None)
    session = AnalysisSession(run_id=run_id, created_at=datetime.now(), config=config, telemetry=TelemetrySink(path=telemetry_path, event_writer=write_event if event_sink is not None else None), handle_store=handle_store, world_store=world_store, llm_provider=llm_provider, embed_provider=embed_provider, node_resolver=node_resolver, artifact_base_dir=artifact_base_dir)
