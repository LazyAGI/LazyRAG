...
def _default_llm_governance() -> ModelGovernanceConfig:
    return ModelGovernanceConfig(on_failure='raise', http_timeout_s=int(os.getenv('EVO_LLM_HTTP_TIMEOUT_S', '300')), producer_timeout_s=float(os.getenv('EVO_LLM_PRODUCER_TIMEOUT_S', '600')))
def _default_embed_governance() -> ModelGovernanceConfig:
    return ModelGovernanceConfig(rate_limit_per_sec=20.0, burst=30, cache_size=512, max_retries=3, retry_base_seconds=2.0, on_failure='disable', http_timeout_s=int(os.getenv('EVO_EMBED_HTTP_TIMEOUT_S', '60')), producer_timeout_s=float(os.getenv('EVO_EMBED_PRODUCER_TIMEOUT_S', '120')))
@dataclass(frozen=True)
...
class DatasetGenConfig:
    ...
    max_workers: int = 5
    task_settings: dict = field(default_factory=lambda : {'single_hop': {'num': 10}, 'multi_hop': {'num': 10}, 'table': {'num': 10}, 'list': {'num': 10}})
@dataclass(frozen=True)
class EvalRunConfig:
    provider: str = ''
    base_url: str = ''
...
def load_config(*, data_dir: Path | None=None, base_dir: Path | None=None, badcase_score_field: str | None=None, code_map_path: Path | None=None) -> EvoConfig:
    ...
    model_config = EvoModelConfig(llm_role=os.getenv('EVO_LLM_ROLE', 'evo_llm'), embed_role=os.getenv('EVO_EMBED_ROLE', 'evo_embed'), auto_user_role=os.getenv('EVO_AUTO_USER_ROLE', 'evo_llm'))
    storage = StorageConfig(base_dir=base_dir)
    storage.ensure()
    analysis = AnalysisConfig(badcase_score_field=score_field, enable_embed_features=os.getenv('EVO_ANALYSIS_ENABLE_EMBED_FEATURES', '').lower() in {'1', 'true', 'yes', 'on'})
    ...
    eval_run = EvalRunConfig(provider=os.getenv('EVO_EVAL_PROVIDER', ''), base_url=os.getenv('EVO_EVAL_BASE_URL', ''), token=os.getenv('EVO_EVAL_TOKEN', ''), mock_path=os.getenv('EVO_EVAL_MOCK_PATH', ''))
    profile = os.getenv('EVO_PROFILE', 'dev')
    cfg = EvoConfig(data_dir=data_dir, storage=storage, default_judge_path=judge_path, default_trace_path=data_dir / 'trace_mock.json', chat_source=chat_source, code_access=code_access, analysis=analysis, model_config=model_config, dataset_gen=dataset_gen, eval_run=eval_run, profile=profile)
    if profile == 'prod':
        missing = []
