...
class Planner:
    ...
    def draft(self, message: str, ctx: PlanContext) -> Intent:
        import uuid
        cap_summary = '\n'.join((f"- {c['op']} (flow={c['flow']}, safety={c['safety']})" for c in ctx.capabilities_with_safety[:20]))
        artifact_hint = ''
        prompt = ''
        raw_answer: Any = None
        source = 'heuristic'
        if (ctx.thread_state or {}).get('pending_checkpoint'):
            ...
            if parsed is None:
                source = 'llm'
                prompt = f"""User message: {message}{artifact_hint}\n\nAvailable operations:\n{cap_summary}\n\nYou are a task planner. Map the user's natural language request to operations.\nRules:\n- For interruption/retry/re-run requests, prefer task.stop_active/task.continue_latest plus the restart op.\n- If user says a dataset is bad and asks to regenerate while analysis is running, emit task.stop_active(flow=run) before dataset_gen.start.\n- '创建评测集/生成数据集' -> dataset_gen.start with kb_id, eval_name\n- '评测/跑评测/生成报告' -> eval.run with dataset_id, target_chat_url; if user mentions dataset_name/数据集名/alias, put it in args.options.dataset_name\n- '分析/诊断' -> run.start with eval_id\n- '修改代码/apply' -> apply.start with report_id. apply already loops code edits and unit tests; do not emit multiple apply.start for one report.\n- 'ABTest/对比' -> abtest.create with apply_id, baseline_eval_id, dataset_id only after the apply is succeeded/accepted and its final unit-test round passed.\n- Extract specific IDs from artifacts when user refers to them\nReply in strict JSON only: {{"ops":[{{"op":"...","reason":"...","args":{{...}}}}],"reply":"..."}}"""
                try:
        ...
        for sel in selected_ops:
            ...
            previews.append(IntentPreview(op=op_name, humanized=sel.get('reason', f'{cap.flow}: {cap.description}'), safety=cap.safety, params_summary=sel.get('args', {})))
        ...
        return Intent(intent_id=f'intent_{ctx.thread_id}_{uuid.uuid4().hex[:8]}', thread_id=ctx.thread_id, user_message=message, reply=reply, suggested_ops_preview=previews, requires_confirm=requires_confirm, thinking=parsed.get('thinking', ''), trace={'source': source, 'prompt': prompt, 'raw_answer': raw_answer, 'parsed': parsed, 'warnings': warnings})
    def materialize(self, intent: Intent, ctx: PlanContext, user_edit: dict | None=None) -> PlanResult:
        ...
        for op_data in raw_ops:
            try:
                ...
                validation_details.append({'op': op_data.get('op') if isinstance(op_data, dict) else None, 'args': op_data.get('args') if isinstance(op_data, dict) else None, 'status': 'rejected', 'error': str(exc)})
        return PlanResult(intent_id=intent.intent_id, ops=ops, warnings=warnings, trace={'raw_ops': raw_ops, 'validation': validation_details})
def _validate_schema(op: str, args: dict[str, Any], ctx: PlanContext) -> None:
    model_by_op = {'run.start': schemas.RunCreate, 'apply.start': schemas.ApplyCreate, 'dataset_gen.start': schemas.DatasetGenCreate, 'eval.run': schemas.EvalCreate, 'eval.fetch': schemas.EvalCreate, 'abtest.create': schemas.AbtestCreate, 'checkpoint.continue': schemas.CheckpointContinue, 'checkpoint.rewind': schemas.CheckpointRewind, 'checkpoint.answer': schemas.CheckpointAnswer, 'checkpoint.cancel': schemas.CheckpointCancel}
...
def _checkpoint_plan(message: str, ctx: PlanContext, llm: Callable[[str], Any]) -> tuple[dict[str, Any], str, Any]:
    ...
    prompt = ''
    for _ in range(2):
        prompt = f"""User message: {message}\n\nPending checkpoint:\n{json.dumps(checkpoint, ensure_ascii=False, indent=2)}\n\nThread state summary:\n{ctx.thread_state_summary}\n\nYou are a checkpoint intent agent. Decide exactly one structured command.\nAllowed commands:\n- checkpoint.continue: user wants to continue the saved next_op.\n- checkpoint.rewind: user wants to rerun from a stage. Args: to_stage in [dataset_gen, eval, run, apply, abtest], optional input_patch for thread inputs such as num_cases, kb_id, algo_id, eval_name, target_chat_url, dataset_name.\n- checkpoint.answer: user asks a question or requests explanation; do not advance.\n- checkpoint.cancel: user wants to stop waiting at this checkpoint.\n\nReturn strict JSON only: {{"ops":[{{"op":"checkpoint.continue|checkpoint.rewind|checkpoint.answer|checkpoint.cancel","reason":"...","args":{{...}}}}],"reply":"user-facing Chinese reply"}}\n{('Previous validation error: ' + last_error) if last_error else ''}"""
        try:
            raw = llm(prompt)
            ...
            if op == 'checkpoint.answer' and not args.get('message'):
                ...
                ops[0]['args'] = args
            model_by_op = {'checkpoint.continue': schemas.CheckpointContinue, 'checkpoint.rewind': schemas.CheckpointRewind, 'checkpoint.answer': schemas.CheckpointAnswer, 'checkpoint.cancel': schemas.CheckpointCancel}
            ...
            last_error = str(exc)
    return ({'ops': [{'op': 'checkpoint.answer', 'reason': '无法将用户消息校验为可执行断点命令', 'args': {'message': f'我还在等待当前断点确认，但无法安全执行这条消息：{last_error}'}}], 'reply': f'我还在等待当前断点确认，但无法安全执行这条消息：{last_error}'}, prompt, raw)
def _validate_checkpoint_boundary(op: str, args: dict[str, Any], ctx: PlanContext) -> None:
    ...
    if op == 'checkpoint.rewind':
        ...
        allowed = {'num_cases', 'kb_id', 'algo_id', 'eval_name', 'target_chat_url', 'dataset_name', 'max_workers', 'filters'}
...
def _validate_abtest_boundary(payload: dict[str, Any], ctx: PlanContext) -> None:
    ...
    for row in state.get('active_tasks') or []:
        ...
        if rp.get('apply_id') == apply_id and rp.get('baseline_eval_id') == payload.get('baseline_eval_id') and (rp.get('dataset_id') == payload.get('dataset_id')) and ((rp.get('eval_options') or {}) == (payload.get('eval_options') or {})):
...
def _heuristic_plan(message: str, ctx: PlanContext) -> dict[str, Any] | None:
    ...
    if any((k in message for k in ('拒绝修改', '拒绝代码', '不接受修改', 'reject apply', 'reject code'))):
        ...
        if task_id:
            return {'ops': [{'op': 'apply.reject', 'reason': '拒绝最近一次代码修改结果', 'args': {'task_id': task_id}}], 'reply': '我会拒绝最近一次代码修改结果。'}
    if any((k in message for k in ('接受修改', '接受代码', '确认修改', '同意修改', 'accept apply', 'accept code'))):
        task_id = _latest_task_id(latest, 'apply')
        if task_id:
            return {'ops': [{'op': 'apply.accept', 'reason': '接受最近一次代码修改结果', 'args': {'task_id': task_id}}], 'reply': '我会接受最近一次代码修改结果。'}
    ...
    if any((k in message for k in ('重试', '续跑', '继续执行', '继续跑', 'retry'))):
        return {'ops': [{'op': 'task.continue_latest', 'reason': '续跑最近暂停或瞬时失败的任务', 'args': {'flow': flow} if flow else {}}], 'reply': '我会续跑最近暂停或瞬时失败的任务。'}
    if any((k in message for k in ('暂停', '停止', '打断', 'stop'))):
        return {'ops': [{'op': 'task.stop_active', 'reason': '暂停当前活跃任务', 'args': {'flow': flow} if flow else {}}], 'reply': '我会暂停当前活跃任务。'}
    if flow == 'eval':
        dataset_id = _extract_id_after(message, ('数据集', '评测集', 'dataset'))
        if dataset_id:
            return {'ops': [{'op': 'eval.run', 'reason': f'用户请求对数据集 {dataset_id} 发起评测', 'args': {'dataset_id': dataset_id}}], 'reply': f'已对数据集 {dataset_id} 发起评测任务。'}
    if flow == 'abtest':
        args = {'apply_id': _extract_named_id(message, 'apply_id') or _extract_id_after(message, ('apply',)), 'baseline_eval_id': _extract_named_id(message, 'baseline_eval_id'), 'dataset_id': _extract_named_id(message, 'dataset_id')}
        ...
        if args['apply_id'] and args['baseline_eval_id'] and args['dataset_id']:
            return {'ops': [{'op': 'abtest.create', 'reason': '按显式参数创建 ABTest', 'args': args}], 'reply': '我会按给定参数创建 ABTest。'}
    if ('数据集' in message or '评测集' in message) and any((k in message for k in ('重新生成', '重生成', '再生成', '生成有问题', '有问题'))):
        ...
        args = {'kb_id': payload.get('kb_id') or _latest_artifact(state, 'kb_id'), 'algo_id': payload.get('algo_id') or 'general_algo', 'eval_name': _regen_name(payload.get('eval_name') or _latest_artifact(state, 'dataset_ids') or 'regen_eval')}
        ...
        if _has_active_flow(active, 'run'):
            ops.append({'op': 'task.stop_active', 'reason': '先暂停当前分析，避免继续基于错误数据集诊断', 'args': {'flow': 'run'}})
...
def _restart_op(flow: str, row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if flow == 'dataset_gen':
        args = {'kb_id': payload.get('kb_id'), 'algo_id': payload.get('algo_id') or 'general_algo', 'eval_name': _regen_name(payload.get('eval_name') or 'regen_eval')}
        if payload.get('num_cases'):
            args['num_cases'] = payload['num_cases']
        return {'op': 'dataset_gen.start', 'reason': '重新生成评测集', 'args': args} if args['kb_id'] else None
    if flow == 'eval':
        args = {'thread_id': row.get('thread_id'), 'dataset_id': payload.get('dataset_id'), 'eval_id': payload.get('eval_id'), 'target_chat_url': payload.get('target_chat_url'), 'options': payload.get('eval_options') or {}}
