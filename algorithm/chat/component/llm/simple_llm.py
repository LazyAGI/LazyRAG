...
class LlmStrategy(BaseModel):
    temperature: Optional[float] = Field(0.01, ge=0.0, le=1.0, description="采样温度，取值范围为[0.0, 1.0]，默认值为0.01")
    max_tokens: Optional[int] = Field(4096, ge=1, le=16048, description="最大token数，取值范围为[1, 16048]，默认值为4096")
    frequency_penalty: Optional[float] = Field(
        0, ge=-2.0, le=2.0, description="重复惩罚，取值范围为[-2.0, 2.0]，默认值为0。正值为减少产生相同token的频率。"
    )
    priority: Optional[int] = Field(
        0, description="请求优先级，用于vllm调度。数值越大优先级越高，默认值为None（使用系统默认优先级）"
    )
    class Config:
        extra = 'allow'
...
class SimpleLlmComponent(ModuleBase):
    ...
    def share(self, prompt: PrompterBase = None, format: FormatterBase = None, stream: Optional[bool] = None,
        ...
        self.llm = self.llm.share(prompt=prompt, format=format, stream=stream, history=history, copy_static_params=copy_static_params)
        return self

    async def astream_iterator(self, input, llm, files, llm_chat_history=[], **kwargs):
        with lazyllm.ThreadPoolExecutor(1) as executor:
            future = executor.submit(llm, input, llm_chat_history=llm_chat_history, lazyllm_files=files, stream_output=True, **kwargs)
            while True:
    ...
    def forward(self, query, files=None, stream=True, **kwargs: Any) -> Any:
        try:
            ...
            if stream:
                response = self.astream_iterator(input=query, llm=llm, files=files, llm_chat_history=llm_chat_history, **kw)
            else:
                response = llm(
                    query, stream_output=False, llm_chat_history=llm_chat_history, lazyllm_files=files, **kw
                )
            return response
