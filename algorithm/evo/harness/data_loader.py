...
def _load_judges(raw_judge: Any) -> tuple[dict[str, JudgeRecord], dict[str, Any] | None, list[str]]:
    """Dispatch judge parsing by format. Returns (judges, eval_meta, warnings)."""
...
def _load_traces(trace_path: Path) -> tuple[TraceMeta, dict[str, TraceRecord], list[str]]:
    """Load traces from file. Returns empty if file missing."""
...
def load_corpus(session: AnalysisSession,
                judge_path: Path | None = None,
                trace_path: Path | None = None) -> LoadSummary:
    """Load judge (+ optional trace) into the session, aligning by trace_id."""
    ...
    trace_missing = [
        f"{did}->{j.trace_id}"
        for did, j in judges.items()
        if traces and j.trace_id not in traces
    ]
    if trace_missing:
        warnings.append(f"{len(trace_missing)} judge cases have no matching trace")

    session.set_parsed_corpus(
        judges=judges, traces=traces, trace_meta=trace_meta,
        warnings=warnings, eval_report_meta=eval_meta,
    )
    return LoadSummary(
