...
def export_case_evidence(dataset_id: str) -> ToolResult[dict[str, Any]]:
    ...
    if session is None:
        return ToolResult.failure('export_case_evidence', ErrorCode.DATA_NOT_LOADED,
                                  'No active session.')
    ...
    step_summaries = {
        n: _module_summary(trace.modules[n])
        for n in pipeline if trace and n in trace.modules
    }
    step_feats = session.case_step_features.get(dataset_id, {})

    return ToolResult.success('export_case_evidence', {
...
def list_cases_ranked(
    ...
    if session is None or not session.parsed_judge:
        return ToolResult.failure('list_cases_ranked', ErrorCode.DATA_NOT_LOADED,
    ...
    return ToolResult.success('list_cases_ranked', {
        'cases': rows[:limit],
        'total_loaded': len(session.parsed_judge),
    })
