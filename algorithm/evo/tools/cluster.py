"""LLM-facing query tools for clustering results.

Compute internals live in :mod:`evo.harness.clustering` and are populated into
session state by the standard plan; these tools are thin readers over that
state, so agents can inspect cluster membership / exemplars / cross-step flow
    ...
    if step_key:
        ...
        if cg is None:
            return ToolResult.failure('get_cluster_summary', ErrorCode.DATA_NOT_LOADED,
                                      'Global clustering not available.')
