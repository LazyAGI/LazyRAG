"""Markdown rendering for the diagnosis report (pure function)."""
...
def json_to_markdown(data: dict[str, Any]) -> str:
    ...
    md: list[str] = [
        "# RAG 诊断报告",
        f"**报告**：`{report_id}` | **时间**：{created_at} | **case 数**：{total} | "
        f"**Synthesizer 轮次**：{iterations}",
        f"**Pipeline**：{pipeline}",
        "",
        "## 核心结论",
        summary or "_（无）_",
    ...
    if not in_scope:
        ...
        sorted_in = sorted(in_scope,
                           key=lambda a: (_priority_rank(a.get("priority")),
                                          -float(a.get("validity_score", 0) or 0)))
