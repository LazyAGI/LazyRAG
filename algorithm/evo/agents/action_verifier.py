...
def _gather_handles(session: AnalysisSession,
                    action: VerifiedAction) -> list[dict[str, Any]]:
...
def _build_user(action: VerifiedAction, evidence: list[dict[str, Any]]) -> str:
    view = {
        "id": action.id, "title": action.title, "rationale": action.rationale,
...
def run_action_verifier(session: AnalysisSession, action: VerifiedAction,
                        *, llm: Any | None = None) -> VerifiedAction:
    ...
    session.telemetry.emit(
        "agent_run", agent=f"{VERIFIER_NAME}:{action.id}", perspective=VERIFIER_NAME,
        action_id=action.id, validity_score=action.validity_score,
...
def verify_actions(session: AnalysisSession, actions: list[VerifiedAction],
                   *, max_workers: int = 4) -> list[VerifiedAction]:
    if not actions:
        return actions
